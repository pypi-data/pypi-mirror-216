...
def _load_hf_model(hf_model_class, model_path, config, **kwargs):
...
def _load_model(path, hf_conf):
    ...
    if hf_conf.get("force_load_config", True):
        ...
        config_class = _get_object_from_module(module_name=hf_conf.get("custom_config_module", "transformers"),
    ...
    if hf_conf.get("force_load_tokenizer", True):
        ...
        tokenizer_class = _get_object_from_module(module_name=hf_conf.get("custom_tokenizer_module", "transformers"),
    ...
    hf_model_class = _get_object_from_module(module_name=hf_conf.get("custom_model_module", "transformers"),
                                             class_name=hf_conf.get("hf_pretrained_class", "AutoModel"))
    hf_model = _load_hf_model(hf_model_class, model_path, config, **sanitize_load_args(model_load_args))
