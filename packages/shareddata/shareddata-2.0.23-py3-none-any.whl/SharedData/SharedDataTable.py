...
class SharedDataTable:
    ...
    def read(self,size=None):
        ...
        if not head_io is None:
            ...
            if nb_head_mb > self.maxtailbytes/(1024*1024):
                if unzip_head:
                ...
                self.shm.buf[nb_hdr:nb_hdr+nb_head] = head_io.read(nb_head)
            ...
            if nb_records_mb > self.maxtailbytes/(1024*1024):                
                ...
                m = hashlib.md5(self.shm.buf[nb_hdr:nb_hdr+nb_records])
    ...
    def upload_head(self,path,nb_header,nb_head,mtime):
        ...
        with gzip.GzipFile(fileobj=gzip_io, mode='wb', compresslevel=1) as gz:
            ...
            if nb_head_mb<=self.maxtailbytes/1000000:
                gz.write(self.shm.buf[nb_header:nb_header+nb_head])
            else:
    ...
    def write_head(self,path,nb_header,nb_head,mtime):
        with open(path/'head.bin', 'wb') as f:
            ...
            if headsize_mb>self.maxtailbytes/1000000:
                ...
                f.write(self.shm.buf[nb_header:nb_header+nb_head])
            f.flush()
