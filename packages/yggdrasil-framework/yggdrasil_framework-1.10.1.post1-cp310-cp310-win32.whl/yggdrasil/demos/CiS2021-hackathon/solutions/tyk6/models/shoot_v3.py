...
if with_yggdrasil:
    ...
    from yggdrasil.languages.Python.YggInterface import YggRpcClient, YggTimesync
...
while t <= tmax:
    ...
    # maximum height of the mesh to the height channel with units and
    # send the current mass and tiem to the timesync channel
    if with_yggdrasil:
        shoot_state = {'mass': units.add_units(mass, 'g')}
        flag, total_state = shoot2root.call(units.add_units(t, 'hrs'),
                                            shoot_state)
        if not flag:
        ...
        # Compute the scale factor using intensity and total mass,
        # stripping units of the result to allow use with trimesh
        # (pretend this is a biologically complex calculation)
        scale = units.get_data(
                total_state['mass'] * intensity /
