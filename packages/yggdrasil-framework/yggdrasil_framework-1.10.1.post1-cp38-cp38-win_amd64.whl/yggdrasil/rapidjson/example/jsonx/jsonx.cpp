...
public:
    ...
    bool Int(int i) {
        char buffer[12];
        return WriteNumberElement(buffer, snprintf(buffer, 12, "%d", i));
    }
    
    bool Uint(unsigned i) {
        char buffer[11];
        return WriteNumberElement(buffer, snprintf(buffer, 11, "%u", i));
    }
    
    bool Int64(int64_t i) {
        char buffer[21];
        return WriteNumberElement(buffer, snprintf(buffer, 21, "%" PRId64, i));
    }
    
    bool Uint64(uint64_t i) {
        char buffer[21];
        return WriteNumberElement(buffer, snprintf(buffer, 21, "%" PRIu64, i));
    }
    
    bool Double(double d) {
        char buffer[30];
        return WriteNumberElement(buffer, snprintf(buffer, 30, "%.17g", d));
    }
