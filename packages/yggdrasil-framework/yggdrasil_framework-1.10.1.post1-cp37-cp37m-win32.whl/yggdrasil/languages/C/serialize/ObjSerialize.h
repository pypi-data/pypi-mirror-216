...
int serialize_obj(const seri_t s, char *buf, const size_t buf_size,
  ...
  if (strlen(p.material) != 0) {
    snprintf(header_format + strlen(header_format), 500 - strlen(header_format), "usemtl %s\n", p.material);
  }
  ...
  for (i = 0; i < p.nface; i++) {
    char ival[10];
    snprintf(iline, 500, "f");
    for (j = 0; j < 3; j++) {
      snprintf(ival, 10, " %d", p.faces[i][j] + 1);
      strcat(iline, ival);
      strcat(iline, "/");
      if (p.face_texcoords[i][j] >= 0) {
	snprintf(ival, 10, "%d", p.face_texcoords[i][j] + 1);
	strcat(iline, ival);
      }
      strcat(iline, "/");
      if (p.face_normals[i][j] >= 0) {
	snprintf(ival, 10, "%d", p.face_normals[i][j] + 1);
	strcat(iline, ival);
      }
    }
