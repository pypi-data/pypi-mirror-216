...
<xsl:template match="text:tab">
    ...
    <xsl:choose>
        ...
        <xsl:when test="parent::text:p[starts-with(@text:style-name,'TEI_5f_linguistic')] or preceding::style:style[@style:name=$currentStyle][starts-with(@style:parent-style-name,'TEI_5f_linguistic')]">
            <xsl:copy-of select="."/>
        </xsl:when>
        <xsl:otherwise/>
    </xsl:choose>
