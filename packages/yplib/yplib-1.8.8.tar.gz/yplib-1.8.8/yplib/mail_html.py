# mail 邮件中的 html 模板
def mail_html(title='title', body='test'):
    s = ['<!DOCTYPE html>',
         '<html>',
         '<head>',
         '    <meta charset="utf-8"/>',
         ...
         '</body>',
         '</html>']
    return list(map(lambda x: x.replace('-title-', ''.join(title)).replace('-body-', ''.join(body)), s))


# 邮件中的 title 的html 代码
def mail_title_html(title='title', body='test', error=False):
    color = 'f35222' if error else '3ecf58'
    s = ['<div style="padding-bottom: 5px;background: white;text-align:center;">',
         ...
         '    </div>',
         '</div>']
    return list(map(lambda x: x.replace('-title-', title).replace('-body-', ''.join(body)).replace('-color-', color), s))
