...
def download_file_from_s3_to_dataset(arquivo, bucket_name, folder_name, sep=';', sheet=0):
        ...
        if arquivo in object_summary.key:
            ...
            if 'xlsx' in object_summary.key:
                df = wr.s3.read_excel(f's3://{bucket_name}/{object_summary.key}', dtype={'CPF': str, 'cpf': str},
                                      boto3_session=session, use_threads=True, engine='openpyxl', sheet=sheet)
...
def get_last_last_modified_file(bucket_name, folder_name, sep=';', sheet=0):
