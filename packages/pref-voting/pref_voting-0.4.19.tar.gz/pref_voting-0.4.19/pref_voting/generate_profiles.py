...
def gen_urn(num_cands, num_voters, replace):
    ...
    for x in range(num_voters):
        flip = np.random.randint(1, ICsize + ReplaceSize)
        if flip <= ICsize:
...
def draw(values, distro):
    ...
    cv = 0
    draw = np.random.random() - distro[cv]
    while draw > 0.0:
        cv += 1
        draw -= distro[cv]
    return values[cv]
...
def generate_truncated_profile(num_cands, num_voters, max_num_ranked=3,probmod="IC"):
    ...
    for r in lprof.rankings:
        ...
        if probmod == "RT":
            truncate_at = np.random.choice(range(1, max_num_ranked + 1))

        if probmod == "IC":
            truncate_at = np.random.choices(range(1, max_num_ranked + 1), weights=probabilities, k=1)[0]
