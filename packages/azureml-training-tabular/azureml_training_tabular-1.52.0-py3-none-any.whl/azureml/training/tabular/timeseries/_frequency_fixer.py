...
from pandas.tseries.offsets import DateOffset, QuarterBegin, QuarterEnd

from azureml._common._error_definition import AzureMLError
from azureml.automl.core.automl_base_settings import AutoMLBaseSettings
from azureml.automl.core.shared._diagnostics.automl_error_definitions import (
    TimeseriesInvalidDateOffset,
...
from ._fixed_dataset import FixedDataSet
from ._freq_aggregator import aggregate_dataset
from ._time_series_column_helper import convert_to_datetime
from ._time_series_data_config import TimeSeriesDataConfig
...
class _CoverageHolder:
    """
    A data class to holding the coverage freq and a valid start of a time series.

    :param freq: the detected frequency of a grain.
    :param start: The detected start of a grain given detected frequency.
    :param grain: The grain name (time series ID).
    :param coverage: How many values are covered by the detected frequency grid.
    """

    def __init__(self, freq: pd.DateOffset, start: pd.Timestamp, grain: GrainType, coverage: int):
        """Constructor."""
        self.grain: GrainType = grain
        self.freq: pd.DateOffset = freq
        self.start: pd.Timestamp = start
        self.coverage: int = coverage


class _FreqStartResults:
    """
    A data class, keeping the results of get_tsdf_frequency_and_start computation.

    :param freq: The imputed data set frequency.
    :param starts: The dictionary or time stamp, containing grain associated with
                   start times.
    :param is_series_healthy_dict: The dictionary, saying that given series do not need to be modified.
    :param is_dataset_healthy: If true all the data set is regular and do not need to be modified.
    """

    def __init__(self,
                 freq: pd.DateOffset,
                 starts: Union[pd.Timestamp, Dict[GrainType, pd.Timestamp]],
                 is_series_healthy_dict: Dict[GrainType, bool],
                 is_dataset_healthy: bool,
                 ):
        """Constructor."""
        self.freq: pd.DateOffset = freq
        self.starts: Union[pd.Timestamp, Dict[GrainType, pd.Timestamp]] = starts
        self.is_series_healthy_dict: Dict[GrainType, bool] = is_series_healthy_dict
        self.is_dataset_healthy: bool = is_dataset_healthy


def get_tsdf_frequency_and_start(
        X: pd.DataFrame,
        y: Optional[CoreDataSingleColumnInputType],
        time_column_Name: str,
        grain_column_names: Optional[Union[List[str], str]],
        user_frequency: Optional[pd.DateOffset] = None
) -> Optional[_FreqStartResults]:
    """
    ...
    freq_offset: Optional[pd.DateOffset] = tsds.infer_freq() if user_frequency is None else None
    # If frequency is inferred, we already have the solution.
    groupby_ob = tsds.data.groupby(tsds.time_series_id_column_names)
    fixed_frequencies_not_nan: Dict[GrainType, _CoverageHolder] = {}
    coverage: int = 0
    dict_starts: Dict[GrainType, pd.DateOffset] = {}
    is_series_healthy_dict: Dict[GrainType, bool] = {}
    is_dataset_healthy = True
    all_freq_unique: bool = True
    previous_freq: Optional[pd.DateOffset] = None
    for grain, df_one in groupby_ob:
        freq_data = improved_infer_freq_one_grain(
            tsds.from_data_frame_and_metadata(df_one, False),
            freq_offset, grain, user_frequency, FREQUENCY_REJECT_TOLERANCE,
            True)
        dict_starts[grain] = freq_data.start
        is_series_healthy_dict[grain] = False
        if freq_data.freq is not None:
            coverage += freq_data.coverage
            is_series_healthy_dict[grain] = freq_data.coverage == df_one.shape[0]
            fixed_frequencies_not_nan[grain] = freq_data
            all_freq_unique = all_freq_unique and previous_freq is None or previous_freq == freq_data.freq
            previous_freq = freq_data.freq
        is_dataset_healthy = is_dataset_healthy and is_series_healthy_dict[grain]
    if not fixed_frequencies_not_nan:
    ...
    if all_freq_unique:
        ...
        if coverage / len(X) >= FREQUENCY_REJECT_TOLERANCE:
            ...
            if len(dict_starts.keys()) == 1 and TimeSeriesInternal.DUMMY_GRAIN_COLUMN in dict_starts.keys():
                starts = dict_starts[TimeSeriesInternal.DUMMY_GRAIN_COLUMN]
            return _FreqStartResults(
                next(iter(fixed_frequencies_not_nan.values())).freq,
                starts,
                is_series_healthy_dict,
                is_dataset_healthy
            )
    return None
...
def improved_infer_freq_one_grain(
        ...
        grain: GrainType,
        user_frequency: Optional[pd.DateOffset] = None,
        custom_freq_reject_tolerance: float = FREQUENCY_REJECT_TOLERANCE,
        is_data_sorted: bool = False) -> _CoverageHolder:
    """
    ...
    :param is_data_sorted: The parameter needed for optimization, if True, we will not sort the grain each time.
    :returns: the holder with frequency, start date and coverage.
    """
    ...
    if len(df_grain.data) == 1:
        ...
        return _CoverageHolder(user_frequency, ix_time.min(), grain, 1)
    ...
    if user_frequency is None:
        ...
        best_coverage = 0
        # If freq is pd.Dateffset, the code below will not work as this offset is not
        # handled properly in pandas.
        if default_frequency.__class__ is not pd.DateOffset:
            try:
                date_grid = pd.date_range(
                    start=user_frequency.rollback(df_grain.time_index.min()),
                    end=user_frequency.rollforward(df_grain.time_index.max()),
                    freq=user_frequency)
            except MemoryError:
                raise ConfigException._with_error(
                    AzureMLError.create(
                        TimeseriesInvalidDateOffset, target="freq",
                        reference_code=ReferenceCodes._FREQUENCY_FIXER_FREQUENCY_INVALID)
                )
            covered = [val in date_grid for val in ix_time]
            best_coverage = sum(covered)
            if best_coverage / len(df_grain.data) > FREQUENCY_REJECT_TOLERANCE:
                return _CoverageHolder(
                    user_frequency, ix_time[np.where(covered)[0].min()], grain, best_coverage)
    ...
    if best_coverage / len(df_grain.data) <= FREQUENCY_REJECT_TOLERANCE or data_grid_too_big:
        ...
        for frequency in frequencues_to_try:
            ...
            if frequency == DateOffset(days=0):
                return _CoverageHolder(default_frequency, ix_time.min(), grain, best_coverage)
            ...
            for i in range(len(anchor_indexes)):
                if old_ix == -1 or anchor_indexes[i] > old_ix + 1:
                    ...
                    if covered_number > best_coverage or (
                            data_grid_too_big and covered_number / len(df_grain.data) > FREQUENCY_REJECT_TOLERANCE):
        ...
        if anchored_freq is not None:
            return _CoverageHolder(anchored_freq, first_timestamp, grain, best_coverage)
...
def fix_df_frequency(X: pd.DataFrame,
                     ...
                     is_series_healthy_dict: Optional[Dict[GrainType, bool]] = None,
                     ) -> pd.DataFrame:
    """
    ...
    :param freq: The frequency detected or set by user.
    :param is_series_healthy_dict: The dictionary, saying that given series do not need to be modified.
    ...
    if not grain_column_names:
        # We do not need to check if the series is healthy as we already know, it is not.
    ...
    for grain, df in X.groupby(groupers, as_index=False, group_keys=False):
        if grain not in start_times.keys() or is_series_healthy_dict and is_series_healthy_dict.get(grain):
...
def check_types(X: pd.DataFrame, y: Optional[CoreDataSingleColumnInputType]) -> None:
    """
...
def fix_data_set_regularity_may_be(
    ...
    was_fixed = False
    if automl_settings.time_column_name in X.index.names:
        timestamps = X.index.get_level_values(automl_settings.time_column_name)
    else:
        timestamps = X[automl_settings.time_column_name]
    if timestamps.isnull().any():
        X = X.copy()
        X[TimeSeriesInternal.DUMMY_TARGET_COLUMN] = y
        X.drop(X[timestamps.isnull()].index, inplace=True, axis=0)
        if X.index.names[0] is None:
            X.reset_index(drop=True, inplace=True)
        y = X.pop(TimeSeriesInternal.DUMMY_TARGET_COLUMN).values
        was_fixed = True

    if results.is_dataset_healthy:
        logger.info('The data set is regular, no fixing needed.')
        return FixedDataSet(
            X, y, False, was_fixed, new_freq)

    if y is not None:
        # If y was provided, align it with X.
        X[TimeSeriesInternal.DUMMY_TARGET_COLUMN] = y
    logger.info('Fixing data set frequency.')
    X_fixed = fix_df_frequency(X,
                               automl_settings.time_column_name,
                               automl_settings.grain_column_names,
                               start_times=results.starts,
                               freq=results.freq,
                               is_series_healthy_dict=results.is_series_healthy_dict)
    logger.info('Frequency fixing is complete.')
    if y is not None:
    ...
    return FixedDataSet(
        X_fixed, y_fixed, False, was_fixed or X_fixed.shape[0] != X.shape[0], new_freq)
