...
class Display:
    ...
    def add_trajectories(self,
                         ...
                         velocity_range: tuple = (None, None),
        ...
        for agent in split_trajectories:
            ...
            if type(color) is list:
                ...
                if len(color) == len(segments):
                    if type(color) is float or type(int) is float:
                        min_velocity = min(color) if velocity_range[0] is None else velocity_range[0]
                        max_velocity = max(color) if velocity_range[1] is None else velocity_range[1]
    ...
    def add_icon(self, location: Location, icon_name: str = "", icon_file_path: str = "", rotation: float = 0, size: float = .05, zorder: int = -5, alpha:float = 1):
        ...
        self.ax.imshow(rotated_img, extent=Agent_markers.icon_box(location, sizepython -v, rotation), alpha=alpha, zorder=zorder)
