...
available_timezones = \
    ["Timezone included in data", "Plant local timezone without DST", "Plant local timezone with DST"] + \
...
def process_tz_string(tz_string: Union[str, None], plant: 'sunpeek.components.Plant' = None):
    if tz_string is None:
        return None
    try:
        return pytz.timezone(tz_string)
    except pytz.exceptions.UnknownTimeZoneError:
        if tz_string == "None (take timezone from uploaded data)":
            return None
        if tz_string == "Plant local timezone without DST":
            if plant is None:
                raise TimeZoneError(f'Time zone "{tz_string}" requires a plant but got a None plant.')
            return pytz.FixedOffset(get_timezone_offset_minutes(plant.latitude, plant.longitude))
        if tz_string == "Plant local timezone with DST":
            if plant is None:
                raise TimeZoneError(f'Time zone "{tz_string}" requires a plant but got a None plant.')
            return pytz.timezone(get_timezone_string(plant.latitude, plant.longitude))
        if 'UTC+' in tz_string:
            return pytz.FixedOffset(int(tz_string.split('+')[1]) * 60)
        if 'UTC-' in tz_string:
            return pytz.FixedOffset(-int(tz_string.split('-')[1]) * 60)
...
def validate_timezone(idx: pd.DatetimeIndex,
    ...
    if isinstance(timezone, str):
        timezone = process_tz_string(timezone, plant)
    if (timezone is not None) and not (isinstance(timezone, pytz.BaseTzInfo) or isinstance(timezone, pytz._FixedOffset)):
