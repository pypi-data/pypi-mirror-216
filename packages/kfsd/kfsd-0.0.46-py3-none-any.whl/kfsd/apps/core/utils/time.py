...
class Time:
    ...
    @staticmethod
    @ExecExceptionHandler(logger)
    def future_time(dateconfig, format="%m/%d/%Y, %H:%M:%S %p"):
        dt = Time.current_time()
        return Time.calculate_time(dt, dateconfig, format)

    @staticmethod
    @ExecExceptionHandler(logger)
    def calculate_time(dt, dateconfig, format="%m/%d/%Y, %H:%M:%S %p"):
        td = timedelta(**dateconfig)
        newtime = dt + td
