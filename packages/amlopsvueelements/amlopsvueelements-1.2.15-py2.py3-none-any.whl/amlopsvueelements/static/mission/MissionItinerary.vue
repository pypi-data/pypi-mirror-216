...
const onDeleteLeg = (leg: IMissionLeg, index: number) => {
  ...
  onDeleteMissionLeg(leg, deletionText)
  changeDepartureAirportOnDelete(index)
}
const changeDepartureAirportOnDelete = (index) => {
  const nextLeg = formData.value?.legs?.[index + 1]
  const prevLeg = formData.value?.legs?.[index - 1]
  if (nextLeg && prevLeg) {
    nextLeg.departure_location = prevLeg.arrival_location
  }
}
watch(
