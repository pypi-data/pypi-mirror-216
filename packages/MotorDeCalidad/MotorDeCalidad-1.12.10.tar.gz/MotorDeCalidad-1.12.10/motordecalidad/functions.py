...
def startValidation(inputspark,config,dfltPath=""):
    ...
    if rules[0] == Rules.Pre_Requisites.code:
        inform("Inicializando regla de requisitos")
        columns = rules[0].get(JsonParts.Fields)
        t = time.time()
    ...
    try:
        ...
        requisitesDf =spark.createDataFrame(data = requisitesData, schema = OutputDataFrameColumns).select(
        DataDate.value(lit("-")),
        CountryId.value(lit("-")),
    ...
    for code in rules:
        if rules[code].get(JsonParts.Fields) not in [0,["0"],"0"] :
            ...
            if code[0:3] == Rules.NullRule.code:
                ...
                threshold:int = rules[code].get(JsonParts.Threshold)
                sign = rules[code].get(JsonParts.sign)
                ...
                t = time.time()
                data, errorDf = validateConditional(object,registerAmount,sign,filterList,entity,threshold,qualityFunction)
