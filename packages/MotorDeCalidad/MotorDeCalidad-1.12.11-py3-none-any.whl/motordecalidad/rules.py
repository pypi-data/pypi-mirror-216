...
from pyspark.sql.functions import to_date,regexp_replace,concat_ws,length,split, lit, collect_list, current_date, datediff, expr, udf
...
def validateRequisites(object:DataFrame, field:list):
    ...
    if len(error_list) == Zero and rowsNumber != Zero :
        print("Ha superado la validación de requisitos exitosamente.\n")
        return  [rowsNumber,Rules.Pre_Requisites.code,Rules.Pre_Requisites.name,Rules.Pre_Requisites.property,Rules.Pre_Requisites.code + "/" + fields,"100","Validación de PreRequesitos Exitosa",fields,"100","0"]
        ...
        return [rowsNumber,Rules.Pre_Requisites.code,Rules.Pre_Requisites.name,Rules.Pre_Requisites.property,Rules.Pre_Requisites.code + "/" + fields,"100","Error en esquema de la tabla",fields,"0",rowsNumber]
    elif rowsNumber == Zero :
        inform("El dataframe de entrada no contiene registros")
        return [rowsNumber,Rules.Pre_Requisites.code,Rules.Pre_Requisites.name,Rules.Pre_Requisites.property,Rules.Pre_Requisites.code + "/" + fields,"100","DataFrame no contiene Registros",fields,"0",rowsNumber]
...
def validateDuplicates(object:DataFrame,fields:List,registersAmount: int,entity: str,threshold: int, repetitionsNumber: int = None):
    
    if repetitionsNumber is not None:
...
def validateConditional(object:DataFrame,
                        registerAmount:int,
                        sign:str,
                        filterLists:List[any],
                        entity:str,
                        threshold:int, 
                        qualityFunction:List[List[any]]):

    column, value = filterLists

    if sign == "in" or sign == "==":
        object = object.filter(object[column].isin(value))
        
    elif sign == "not in":
        object = object.filter(object[column] != (value))
    ...
    newRegisterAmount = object.count()

    functionName = qualityFunction[0]
    qualityFunction[2] = newRegisterAmount
    ...
    dataRequirement = f"Para {column} = {value}. " + data[-4]
    columnName = qualityFunction[1]
    errorCount = data[-One]
    ratio = data[-2]

    return [newRegisterAmount,Rules.validateConditional.code,Rules.validateConditional.name,Rules.validateConditional.property,Rules.validateConditional.code + "/" + entity + "/" + columnName,threshold,dataRequirement, columnName, ratio, errorCount], errorDf
