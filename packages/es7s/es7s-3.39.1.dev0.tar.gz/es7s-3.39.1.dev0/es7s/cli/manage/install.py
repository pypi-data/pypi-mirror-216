...
from es7s.shared import Styles, format_attrs, get_logger, get_stdout
...
from .._decorators import (_catch_and_log_and_exit, _preserve_terminal_state, _with_progress_bar,
                           cli_command,
                           cli_option)
from .._invoker import AutoInvoker
from es7s.shared.iterx import countindex
...
class InstallCommand:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        InstallCommand.pbar = pbar
        self._dry_run = dry_run
        self._symlinks = symlinks
        self._run()

    def _run(self):
        InstallCommand.pbar.setup(len(self._stages))
        for stage_fn, stage_desc in self._stages.items():
            ...
            InstallCommand.pbar.next_threshold(stage_desc)
            try:
                result_msg = stage_fn()
    ...
    def _run_build_install_tmux(self) -> str:
        ...
        try:
            ...
            InstallCommand.pbar.set((idx := idx + 1), label_local="Cloning primary repo")
            self._clone_git_repo("ssh://git@github.com/dl-forks/tmux", temp_dir_path)

            InstallCommand.pbar.set((idx := idx + 1), label_local="Building from sources")
            ...
            InstallCommand.pbar.set((idx := idx + 1), label_local="Cloning tpm repo")
            self._clone_git_repo("ssh://git@github.com:tmux-plugins/tpm", tpm_dir_path)

            InstallCommand.pbar.set((idx := idx + 1), label_local="Installing tpm")
...
class AbstractRunner(metaclass=ABCMeta):
    ...
    def _clone_git_repo(self, remote_url: str, path: str) -> bool:
        ...
        invoker = AutoInvoker('git')
        invoker.spawn("clone", remote_url, path, "--progress")
        return True
...
class CopyDataRunner(AbstractRunner):
    def _run(self) -> str:
        ...
        dist_dir = importlib.resources.files(APP_NAME).joinpath(dist_dir_relpath)
        ...
        total_actual = 0
        for idx, total, dist_relpath in countindex(dist_dir.iterdir()):
            InstallCommand.pbar.set(idx / total, label_local=str(dist_relpath))
