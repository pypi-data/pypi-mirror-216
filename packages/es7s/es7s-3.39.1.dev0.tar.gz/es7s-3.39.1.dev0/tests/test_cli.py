...
class TestCommand:
    @pytest.mark.parametrize(
        ...
        [
            #["exec describe es7s", "es7s.cli.__main__"],
            ...
            #["exec esqdb send /dev/zero /dev/zero", re.compile("Dusk|Now")],
    ...
    def test_command_invocation(
        ...
        if not stdout:
            # oops! looks like our output bypassed io proxy. but it was
            # (hopefully) captured by one-level-above pytest runner, so lets
            # get it from there. most likely this happens when external
            # component like shell script is invoked using AutoInvoker.
            sys.stdout.seek(0)
