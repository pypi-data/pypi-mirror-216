...
from es7s.shared import get_config, get_logger, run_subprocess
...
class EditImageCommand:
    ...
    def _run(self, path: click.Path):
        ...
        ext_vector: list[str] = get_config().get(self._config_section, "ext-vector")
        editor_type = "raster"
        if ext.removeprefix(".") in ext_vector:
            editor_type = "vector"
        get_logger().debug(f"Selected editor type: {editor_type}")

        editor = get_config().get(self._config_section, f"editor-{editor_type}")
