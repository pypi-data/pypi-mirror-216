...
from es7s.shared.demo import DemoGradients
from es7s.shared.gradient import GimpGradientReader, GradientPoint
...
class IDetailsRenderer(metaclass=ABCMeta):
    ...
    def _get_details(self, gp: GradientPoint):
        return [
            ...
            "#{:06x}".format(pt.rgb_to_hex(gp.col)),
...
class DemoGradientCommand:
    ...
    def _format_scale(self) -> Iterable[pt.Fragment]:
        ...
        for idx, c in enumerate(range(self._width)):
            ...
            result_frag.append(
                pt.Fragment(" ", FrozenStyle(fg="gray70", bg=ColorRGB(pt.rgb_to_hex(col))))
            )
        return result_frag
