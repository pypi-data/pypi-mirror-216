...
from es7s.shared.path import RESOURCE_DIR
...
class _BoolState(_StaticState):
    ...
    def _update_config(self, val: bool):
        if self.config_var is None:
            return
        if self.config_var_value is None:
            rewrite_user_config_value(*self.config_var, "on" if val else "off")
        else:
            if not val:
                return
            rewrite_user_config_value(*self.config_var, self.config_var_value)
...
class _BaseIndicator(ShutdownableThread, t.Generic[DT], ABC):
    ...
    def __init__(
        ...
        self._theme_version = get_config().get("indicator", "theme-version")
        self._theme_path = importlib.resources.files(APP_NAME).joinpath(
            Path(RESOURCE_DIR, "icons-v" + self._theme_version)
        )
