...
from es7s.shared import (ConfigLoaderParams, FrozenStyle, IoParams, LoggerParams, destroy_io,
                         destroy_logger,
                         get_logger, get_stderr, get_stdout, init_config, init_io, init_logger)
...
from ._decorators import _catch_and_log_and_exit, cli_group, cli_option, cli_pass_context
...
def _audit_event(ev: str, *args):
    match ev.split('.')[0]:
        case 'os'|'subprocess'|'shutil'|'tempfile'|'importlib':
            get_logger().log_audit_event(ev, *args)
            return
