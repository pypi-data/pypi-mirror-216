...
LOGGING_CONFIG = {
    ...
    "formatters": {
        "default": {
            "format": "%(asctime)s :%(name)s:data-ecosystem-services:%(module)s:%(lineno)d " "%(levelname)s %(message)s",
            "datefmt": "%Y-%m-%d %H:%M:%S",
        },
        "simple": {
            "format": "%(message)s",
...
class LoggerSingleton:
    ...
    @staticmethod
    def instance(calling_namespace_name=None, calling_service_name=None):
        ...
        if LoggerSingleton._instance is None:
            LoggerSingleton(calling_namespace_name=None,
                            calling_service_name=None)
        return LoggerSingleton._instance

    def __init__(self, calling_namespace_name=None, calling_service_name=None):
    ...
    def error_with_exception(self, message, exc_info):
        ...
        Raises:
            TypeError: If exc_info is not a tuple or does not have three elements.
        """
        self.logger.error("%s: %s", message, sys.exc_info()[1])
    ...
    def get_datetime(self, entry):
        ...
        try:
            ...
            print(
                f"Could not parse datetime from entry: {entry}. Error: {str(ex)}")
            return datetime.min
