...
class LoggerSingleton:
    ...
    def get_log_file_tail(self, number_of_lines=100):
        """
        Read the last number_of_lines from the log file.
        ...
        Returns:
        str: The last number_of_lines of the log file.
        """
        try:
            with open(LOG_FILENAME, 'r') as file:
                lines = file.readlines()

                # Get the last number_of_lines and reverse them
                last_lines = lines[-number_of_lines:][::-1]

                # Combine the lines into a single string
                return ''.join(last_lines)

        except FileNotFoundError:
            print(f"File {LOG_FILENAME} not found.")
            return None
