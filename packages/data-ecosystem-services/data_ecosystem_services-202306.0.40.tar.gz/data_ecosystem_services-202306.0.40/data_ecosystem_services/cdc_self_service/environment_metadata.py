...
NAMESPACE_NAME = os.path.basename(__file__)
...
AZURE_CLIENT_ID {sp_client_id}
    ...
    def get_configuration_common(cls, parameters: dict, dbutils) -> dict:
        ...
        if os_name_check.lower() == "nt":
            print("windows: nt")
            repository_path = cls.convert_to_windows_dir(repository_path)

        env_folder_path = f"{repository_path.rstrip('/')}/{project_id_root}/{project_id}/{config_string}"
        environment_json_path = f"{env_folder_path}/{config_string}.{environment}.json"
        environment_json_path_default = f"{env_folder_path}/{config_string}.{environment}.json"
        ...
        if not cls.file_exists(running_local, environment_json_path, None):
            print(f"config does not exist: {environment_json_path}")

            # Try parent folder if environment_json_path doesn't exist
            repository_path_temp = os.getcwd()
            repository_path_temp = str(Path(repository_path_temp))
            repository_path_temp = cls.convert_to_windows_dir(
                repository_path_temp)
            repository_path_temp = f"{repository_path_temp}{project_id_root}/{project_id}/{config_string}/"
            environment_json_path = repository_path_temp + \
                f"{config_string}.{environment}.json"
            print(f"environment_json_path check 2: {environment_json_path}")

            if not cls.file_exists(running_local, environment_json_path, None):
                print(f"config does not exist: {environment_json_path}")

                # Try current folder if parent folder doesn't have config
                repository_path_temp = os.getcwd()
