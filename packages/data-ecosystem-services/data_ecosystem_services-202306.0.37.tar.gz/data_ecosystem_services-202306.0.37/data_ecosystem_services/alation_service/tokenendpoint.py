...
class TokenEndpoint:
    ...
    def get_api_token_from_config(self, config):
        ...
        try:
            ...
            with tracer.start_as_current_span(f"get_api_token_from_config"):
                ...
                logger.info(
                    f"api_access_token_length:{str(len(api_access_token))}")
                status_code = 200
                return status_code, api_access_token
    ...
    def create_api_access_token_via_refresh(edc_alation_base_url, edc_alation_user_id, edc_alation_refresh_token):
        ...
        with tracer.start_as_current_span("create_api_access_token_via_refresh"):
            ...
            if response.status_code in (200, 201):
                ...
                api_access_token = response_data.get(
                    "api_access_token", "not_set")
                logger.info(
                    f"Generated API response length: {len(api_access_token)}")
            else:
                logger.error("Failed to generate API token:" + str(response))
