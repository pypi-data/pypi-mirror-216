...
class Configuration:
    def __init__(self):
        ...
        self.default_timeout: int = int(os.environ.get("SHIPPO_DEFAULT_TIMEOUT", 80))
        self.rates_req_timeout = float(os.environ.get("SHIPPO_RATES_REQ_TIMEOUT", 20.0))

        # SETTINGS BELOW APPLY ONLY TO TESTS

        # Controls how much information is logged to stdout
        self.vcr_logging_level = os.environ.get("SHIPPO_VCR_LOGGING_LEVEL", "ERROR")
        # Switch to 'all' for re-recording all cassettes (to be used in CI so that all tests are run without mocks)
        self.vcr_record_mode = os.environ.get("SHIPPO_VCR_RECORD_MODE", "once")
