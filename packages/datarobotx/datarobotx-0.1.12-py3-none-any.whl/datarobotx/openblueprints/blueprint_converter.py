...
from datarobotx.openblueprints.blueprint_mappings import CONVERTER_MAPPINGS, LOOKUP_MAPPINGS
...
if TYPE_CHECKING:
    StageDict = Dict[str, Sequence[str]]
...
class BlueprintConverter(BlueprintStringConverter):
    ...
    def convert(cls, blueprint_json: BlueprintJson) -> str:
        ...
        construct_stage_input_paths = cls.construct_stage_input_paths(stages, estimator_stage_key)

        # Construct a preprocessing pipeline from the stage mappings
        preprocessor_pipeline = cls.build_preprocessor_pipeline(stages, construct_stage_input_paths)
    ...
    def extract_stages(cls, blueprint_json: BlueprintJson) -> StagesDict:
        stages = {}
        for stage in sorted(blueprint_json.keys()):
            dr_class, class_str = cls.parse_blueprint_stage(blueprint_json[stage])
            stages[stage] = {
                "name": dr_class,
                "transform": class_str,
                "input": blueprint_json[stage][0],
            }
        return stages
    ...
    @classmethod
    def parse_blueprint_stage(cls, blueprint_stage: BlueprintStage) -> Tuple[str, str]:
        """Convert a stage of a blueprint to an open representation mapping"""
        _, stage_class, _ = blueprint_stage
        ...
        return cls.convert_blueprint_class_to_open_representation(dr_class, dr_class_params)
    ...
    def convert_blueprint_class_to_open_representation(
        ...
        converter_class = LOOKUP_MAPPINGS.get(dr_class)
        if converter_class is not None:
            class_str = CONVERTER_MAPPINGS[converter_class].get_open_representation(dr_class_params)
        else:
            class_str = PASSTHROUGH_STRING
        return dr_class, class_str
