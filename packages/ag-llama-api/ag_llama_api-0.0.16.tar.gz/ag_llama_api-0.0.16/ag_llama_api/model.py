...
class StreamModel:
    ...
    def tokenize(self, text):
        ...
        if self.path:
            s_tokenizer =SentencePieceProcessor(model_file=f"{self.path}/tokenizer.model")
            num_tokens = len(s_tokenizer.EncodeAsIds(text))
            logger.debug(f"Full tokenized text={num_tokens} tokens")
        batch = self.tokenizer.encode(text,truncation=True, return_tensors="pt")
        logger.debug(f"Tokenized text={batch.shape[-1]} tokens")
        self.wait_and_clear_gpu()
        logger.debug("Sending tokenized text to GPU")
    ...
    def wait_and_clear_gpu(self):
        ...
        for i,handle in enumerate(handles):
            info=str(pynvml.nvmlDeviceGetMemoryInfo(handle))
...
def gpu_sum(info):
    bytes_mib = 1024.0 ** 2
    s=""        
    s+='used:'+f"{int(info.used / bytes_mib)} MiB"
    s+='free'+ f"{int(info.free / bytes_mib)} MiB"
    s+='total'+f"{int(info.total / bytes_mib)} MiB"
    return s