...
class IAMRoles(StackTemplate):
    def __init__(
        ...
        if role_config.is_enabled() == False:
            self.init_template('IAM Roles: Roles and Instance Profiles')
            return
...
"""
        ...
        assume_role_policy_statement = """          - Effect: "Allow"
            Principal:{0[assume_role_principal]:s}
            Action:
              - "sts:AssumeRole"
            {0[assume_role_condition]:s}
"""
        assume_role_with_web_identity_policy_statement = """          - Effect: "Allow"
            Principal:{0[assume_role_principal]:s}
            Action:
              - "sts:AssumeRoleWithWebIdentity"
"""
        assume_role_with_web_identity_policy_condition = """            Condition:
              "StringEquals":
                "token.actions.githubusercontent.com:aud": "sts.amazonaws.com"
              "StringLike":
                "token.actions.githubusercontent.com:sub": "{0[repository_filter]:s}"
"""
...
"""
        ...
        assume_role_statement_yaml = ""
        assume_role_statements = []
        # if role_config.paco_ref_parts == 'resource.iam.roles.vanta-prod-read-only':
        #     breakpoint()
        if role_config.assume_role_policy != None:
            assume_role_statement_yaml = f"\n{assume_role_policy_statement}"
            ...
            iam_role_table['assume_role_condition'] = ''
            if role_config.assume_role_policy.condition != None:
                iam_role_table['assume_role_condition'] = f"Condition: {role_config.assume_role_policy.condition}\n"

            assume_role_statements.append(assume_role_statement_yaml.format(iam_role_table))

        elif role_config.assume_role_policies != None:
            # Multi-Statement Assume Policy
            assume_role_statement_yaml = ""
            for assume_role_policy_statement in role_config.assume_role_policies.statement:
                if len(assume_role_policy_statement.principal.federated) > 0:
                    principal_yaml = f"""
              Federated: {assume_role_policy_statement.principal.federated}"""
                iam_role_table['assume_role_principal'] = principal_yaml

                assume_role_statement_yaml += assume_role_with_web_identity_policy_statement.format(iam_role_table)
                if assume_role_policy_statement.condition:
                    assume_role_statement_yaml += f"            Condition: {assume_role_policy_statement.condition}\n"
                assume_role_statements.append(assume_role_statement_yaml)
        elif role_config.identity_provider != None:
            idp_role_config = role_config.identity_provider
            ...
            for repo_filter in idp_role_config.repository_filters:
                iam_role_table['repository_filter'] = repo_filter
                assume_role_statement_yaml += assume_role_with_web_identity_policy_statement.format(iam_role_table)
                assume_role_statement_yaml += assume_role_with_web_identity_policy_condition.format(iam_role_table)
                assume_role_statements.append(assume_role_statement_yaml)
        else:
            pass

        for assume_role_statement in assume_role_statements:
            iam_role_table['assume_role_statements'] += assume_role_statement
        ...
        if role_config.policies and role_config.is_enabled():
            iam_role_table['inline_policies'] = self.gen_role_policies(role_config)
        else:
    ...
    def gen_iam_role_name(self, role_type, role):
        ...
        if role.global_role_name == True:
            role_name = f'{role.role_name}-{role_type[0]}'
            role_ref = Reference(role.paco_ref)
            if role_ref.parts[0] == 'netenv' and role_name.find('<environment>') > -1:
                role_name = role_name.replace('<environment>', role_ref.parts[2])
            return role_name
    ...
    def gen_role_policies(self, role_config):
...
"""
        ...
        unquoted_list_fmt = """
                  - {0}"""
        role_ref = Reference(role_config.paco_ref)
        role_environment = None
        if role_ref.parts[0] == 'netenv':
            role_environment = role_ref.parts[2]
        for inline_policy in role_config.policies:
            policy_table['name'] = ""
            ...
            for statement in inline_policy.statement:
                ...
                for resource in statement.resource:
                    resource_new = resource.replace('<account>', self.account_ctx.id)
                    resource_new = resource_new.replace('<region>', self.aws_region)
                    if role_environment != None:
                        resource_new = resource_new.replace('<environment>', role_environment)
                    if resource_new[0:1] == '!':
                        statement_table['resource_list'] += unquoted_list_fmt.format(resource_new)
                    else:
                        statement_table['resource_list'] += quoted_list_fmt.format(resource_new)
