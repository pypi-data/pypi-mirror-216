...
class ECSServices(StackTemplate):
    def __init__(self, stack, paco_ctx, task_execution_role):
        ...
        for task in ecs_config.task_definitions.values():
            ...
            for container_definition in task.container_definitions.values():
                ...
                merged_environment = {}
                if ecs_config.setting_groups != None:
                ...
                if references.is_ref(container_definition.image):
                    ...
                    image_arn_param = self.create_cfn_parameter(
                        ...
                        ignore_changes=container_definition.ignore_image_changes
                    )
                    param_name = self.create_cfn_logical_id(f'{task.name}{container_definition.name}ImageTag')
                    image_tag_param = self.create_cfn_parameter(
                        param_type='String',
                        name=param_name,
                        description=f'Tag of the image to deploy into the container.',
                        value=container_definition.image_tag,
                        ignore_changes=container_definition.ignore_image_changes
                    )
                    ...
                    task_dict['ContainerDefinitions'][index]['Image'] = troposphere.Join(
                        ':', [
                            ...
                            troposphere.Ref(image_tag_param) # Image tag
                        ]
                    )
                else:
                ...
                if getattr(container_definition, 'logging') != None:
                    ...
                    if container_definition.logging.driver == 'awslogs':
                        ...
                        if container_definition.logging.awslogs_one_cluster_group == True:
                            # Consolidates all Service Logs into one LogGroup
                            prefixed_log_group_name = prefixed_name(container_definition, None, None, False)
                            prefixed_log_group_name = prefixed_log_group_name + '-ECS-Cluster'
                        elif container_definition.logging.awslogs_resource_app_name != None:
                            # Used to consolidate Blue/Green Service LogGroups
                            prefixed_log_group_name = prefixed_name(container_definition, None, None, False, container_definition.logging.awslogs_resource_app_name)
                            prefixed_log_group_name += f'-ECS-Service-{task.name}'
                        else:
                            prefixed_log_group_name = prefixed_name(container_definition, task.name)
                        if container_definition.logging.awslogs_create_log_group == True:
                            ...
                            log_dict['Options']['awslogs-group'] = prefixed_log_group_name
        ...
        for service in ecs_config.services.values():
            ...
            desired_tasks_param = self.create_cfn_parameter(
                ...
                value=desired_tasks,
                ignore_changes=service.ignore_capacity_changes
            ...
            # Service Discovery
            if service.hostname != None and private_dns_namespace_res != None:
            ...
            deploy_min_healthy_percent_param = self.create_cfn_parameter(
                name=self.create_cfn_logical_id(f"{cfn_service_name}DeployMinHealthyPercent"),
                param_type='String',
                description='The minimum number of healthy tasks during deployment',
                value=service.deployment_minimum_healthy_percent,
                ignore_changes=service.ignore_capacity_changes
            )
            service_dict['DeploymentConfiguration'] = {
                "MaximumPercent": service.deployment_maximum_percent,
                "MinimumHealthyPercent": troposphere.Ref(deploy_min_healthy_percent_param)
            }
            ...
            if uses_target_tracking_scaling:
                ...
                suspend_scaling_param = self.create_cfn_parameter(
                    param_type='String',
                    name=self.create_cfn_logical_id(f'Service{service.name}SuspendScaling'),
                    description='Boolean indicating whether the services scaling policies are enabled.',
                    value=suspend_scaling,
                    #ignore_changes=True
                )
                scalable_target_dict['SuspendedState'] = {
                    "DynamicScalingInSuspended" : troposphere.Ref(suspend_scaling_param),
                    "DynamicScalingOutSuspended" : troposphere.Ref(suspend_scaling_param),
                    "ScheduledScalingSuspended" : troposphere.Ref(suspend_scaling_param),
                }
            ...
            for target_tracking_policy in service.target_tracking_scaling_policies.values():
                ...
                # ScalingPolicies
                scaling_policy_dict = {}
                scaling_policy_dict['PolicyName'] = self.create_cfn_logical_id(f"{self.stack.get_name(self)}{target_tracking_policy.name}Policy")
                ...
                # Used by other services to disable scaling when disabling ECS services
                disable_scale_in_param = self.create_cfn_parameter(
                    param_type='String',
                    name=self.create_cfn_logical_id(f'Service{service.name}{target_tracking_policy.name}DisableScaleIn'),
                    description='Boolean indicating whether the services scaling policies are enabled.',
                    value=target_tracking_policy.disable_scale_in,
                    #ignore_changes=True
                )

                scaling_policy_dict['TargetTrackingScalingPolicyConfiguration'] = {
                    "DisableScaleIn": troposphere.Ref(disable_scale_in_param),
    ...
    def add_log_group(self, loggroup_name, expire_events_after_days):
        ...
        if loggroup_name in self.loggroup_cache.keys():
            return self.loggroup_cache[loggroup_name]
        ...
        self.loggroup_cache[loggroup_name] = loggroup_resource
