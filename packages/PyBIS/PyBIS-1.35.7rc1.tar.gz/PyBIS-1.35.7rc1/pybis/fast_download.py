...
class FastDownload:
    ...
    def _download_step(self, download_url, download_session_id, session_stream_ids, ranges,
        ...
        total_chunks_count = len(chunks_to_download)
            ...
            while True:  # each iteration will create threads for streams
                ...
                if chunks_to_download == set():  # if there are no more chunks to download
                    ...
                    if counter >= DOWNLOAD_RETRIES_COUNT:
                        ...
                        break
                    # if no chunks of files over 10 MB could be downloaded, break the whole download
                    # This is done to limit amount of retry calls to server
                    if 10 < total_chunks_count == len(chunks_to_download):
                        print(f"There was a problem during download. Please check your connection.")
                        exception_list += [
                            ValueError(f"There was a problem during download. Please check your connection.")]
                        break
