...
model_name = os.environ.get('model_name',
                            'aif_proj_boost_norm_log_rel_dyn_aux_dis_delayed_decay_exp')                     # Model name

# Dataset path, file's name and outputs directory name
root = f'{work_dir}/batch_effects_biomed/attribute-cVAEGAN/InData'
data_file = f'{work_dir}/batch_effects_biomed/data_preprocessed/{file_name}_norm_log.csv'
...
log_transfo = True              # Whether to perform log1p transformation
...
robust_grid_search(model=model, lr_list=lr_list, batch_size_list=batch_size_list,
                   ...
                   max_epochs=epochs, projection_constraint=projection_constraint, scheduler=scheduler,
                   start_decay=start_decay, decay_factor=decay_factor, update_dis_freq=update_dis_freq, update_aux_freq=update_aux_freq,
                   label_projection=label_projection, n_samplings=n_samplings, n_labels=n_labels,
