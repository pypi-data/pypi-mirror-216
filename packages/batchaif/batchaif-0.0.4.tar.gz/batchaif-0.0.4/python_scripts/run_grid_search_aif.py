...
model_name = os.environ.get('model_name', 'aif')         # Model name
...
hyperparameters_file = f'{out_dir}/fine_tuned_hyperparameters_set.csv'
...
log_transfo = False             # Whether to use log1p transformation on counts
...
weighting_strategy = 'rel'      # Weighting strategy to use for balancing the losses
class_weights = None            # Weights for weighted cross entropy in case of unbalanced dataset
dynamic_ratio = 1.0             # Percentage of the data to use for dynamic reweighting
...
grid_search(model=model, lr_list=lr_list, batch_size_list=batch_size_list, max_epochs=epochs, beta_1_list=beta_1_list,
            beta_2_list=beta_2_list, adata=adata, root=root, file_name=file_name, model_name=model_name,
            results_path=out_dir, losses_enc_weights=losses_enc_weights, weighting_strategy=weighting_strategy,
            losses_norm=losses_norm, seed=seed, norm_data=norm_data, log_transfo=log_transfo,
            label_projection=label_projection, n_labels=n_labels,
