...
model_name = os.environ.get('model_name', 'aif_proj_boost_rel_dyn_aux_dis_delayed_decay_exp_longer')  # Model name

# Dataset path, file's name and outputs directory name
root = f'{work_dir}/InData'
data_file = f'{work_dir}/data_preprocessed/{file_name}.csv'
experiment_path = f'{work_dir}/Experiments'
results_path = f'{experiment_path}/{file_name}'
out_dir = f'{results_path}/single_run_analysis'
hyperparameters_file = f'{out_dir}/proj_fine_tuned_hyperparameters_set.csv'
...
log_transfo = False                 # Whether to use the log1p transformation on counts
...
projection_constraint = {'random': True,    # Projection constraint on reconstructed features using random batch label
                         'avg': True}       # Projection constraint on average batch label
...
scheduler = 'exponential'           # Whether to use exponential LR scheduler
decay_factor = 0.999                # Decay factor for exponential LR scheduler
start_decay = int(0.2 * epochs)     # Epoch when to start lr exponential decay
...
grid_search(model=model, lr_list=lr_list, batch_size_list=batch_size_list, max_epochs=epochs,
            ...
            class_weights=class_weights, grad_clip=grad_clip, projection_constraint=projection_constraint,
            update_aux_freq=update_aux_freq, update_dis_freq=update_dis_freq, scheduler=scheduler,
