...
class PyxetCLI:
    ...
    @staticmethod
    @cli.command()
    def rm(paths: Annotated[list[str], typer.Argument(help="File or folder which will be deleted")],
           message: Annotated[
               str, typer.Option("--message", "-m", help="A commit message")] = ""):
...
class RepoCLI:
    ...
    def fork(source: Annotated[str, typer.Argument(help="origin repo to fork from")],
             dest: Annotated[str, typer.Argument(help="new repository name")],
             private: Annotated[bool, typer.Option('--private', '-p', help="new repository name")] = False,
             ):
        """
