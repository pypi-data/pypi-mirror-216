...
def map(lst, fn, batch_size=10):
    ...
    def map_thread(i):
        print(f"Map {i + 1} of {len(lst)} starting")
        result = fn(lst[i])
        print(f"Map {i + 1} of {len(lst)} received")
        with lock:
            results[i] = result
        print(f"Map {i + 1} of {len(lst)} complete")

    # Initialize the list that will store our threads
    threads = []

    for i in range(0, len(lst), batch_size):
        print(f"Batch {i // batch_size + 1} of {len(lst) // batch_size + 1}")
