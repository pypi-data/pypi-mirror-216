...
def create(db, resource_type, **resource):
    ...
    if resource_type == "ragt-ag-project":
        ...
        #     then: url=[/2021/3/5...]
        post_type = mf.util.discover_post_type(resource)
        slug = None
        if post_type == "article":
            slug = resource["name"][0]
        elif post_type == "listen":
            result = youtube_search.YoutubeSearch(
                resource["listen-of"][0], max_results=10
            ).to_dict()[0]
            web.enqueue(
                webint_media.download, f"https://youtube.com/watch?v={result['id']}"
            )
