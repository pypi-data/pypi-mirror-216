...
from collections.abc import Collection, AsyncGenerator, Generator
...
class Mongo(Database):
    ...
    async def get_admin(self, collection: str, mongoattributes=None):
        """
    ...
    async def upsert_admin(self, obj: root.DesktopObject, collection: str, default_content: Optional[IO] = None):
        """
        ...
        coll = self.get_collection(collection)
        try:
            result = await coll.replace_one({'_id': ObjectId(obj.id)}, obj.to_dict(), upsert=True)
            if result and default_content is not None:
                fs = self.__new_gridfs_bucket(collection)
                await fs.upload_from_stream_with_id(ObjectId(result.upserted_id), obj.display_name, default_content)
            return str(result.inserted_id)
        except NoFile:
            logger.exception('Error inserting %s into collection', obj, collection);
            return None

    async def insert_admin(self, obj: root.DesktopObject, collection: str, default_content: Optional[IO] = None):
        """
        Insert a desktop object into the database, with its content if provided, and with no permission checking.
        ...
        try:
            ...
            if result and default_content is not None:
                fs = self.__new_gridfs_bucket(collection)
                await fs.upload_from_stream_with_id(ObjectId(result.inserted_id), obj.display_name, default_content)
            return str(result.inserted_id)
        except (pymongo.errors.DuplicateKeyError, NoFile) as e:
            logger.exception('Error inserting %s into collection', obj, collection);
            return None

    async def update_admin(self, obj: root.DesktopObject, collection: str, default_content: Optional[IO] = None):
        """
