...
namespace ale {
    ...
    {
        using Ts::operator()...;
        make_visitor(Ts&& ...ts) : Ts{std::forward<Ts>(ts)}...
        {}
     ...
     */
    template <typename TVisitor, typename TType>
    decltype(auto) evaluate_function(TVisitor&& visitor, function_node<TType>* node, symbol_table& symbols) {
    ...
    namespace helper {
        ...
        struct extract_function_arguments_visitor {
            extract_function_arguments_visitor(std::vector<value_node_variant>& children): children(children) {}
