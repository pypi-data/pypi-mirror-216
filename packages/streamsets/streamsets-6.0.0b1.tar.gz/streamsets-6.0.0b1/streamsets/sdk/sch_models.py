...
from .models import Configuration as LegacyConfiguration
    ...
    """
        Args:
            ...
            supported_connection_types (:obj:`list`, optional): List of :obj:`str` instances of supported connection
                types. Default: ``None``
    """
    def __init__(self, stage, pipeline=None, label=None, supported_connection_types=None):
        self._supported_connection_types = supported_connection_types
        self._pipeline = pipeline
        super().__init__(stage, label)
...
class PipelineBuilder(SdcPipelineBuilder):
    ...
    def remove_stage(self, label=None, name=None, type=None, library=None):
        ...
        Returns:
            An instance of :py:class:`streamsets.sdk.sch_models.SchSdcStage`.
        """
        stage_instance = next(stage.instance for stage in
                              self._get_stage_data(label=label, name=name, type=type, library=library)
                              if stage.definition.get('errorStage') is False)
        for index, stage in enumerate(self._pipeline[self._config_key]['stages']):
            if stage['stageName'] == stage_instance['stageName']:
                self._pipeline[self._config_key]['stages'].pop(index)

                # Disconnect inputs & outputs
                for pipeline_stage in self._pipeline[self._config_key]['stages']:
                    if stage['inputLanes']:
                        index = 0
                        while index < len(pipeline_stage['outputLanes']):
                            if pipeline_stage['outputLanes'][index] in stage['inputLanes']:
                                pipeline_stage['outputLanes'].pop(index)
                            else:
                                index += 1

                    if stage['outputLanes']:
                        index = 0
                        while index < len(pipeline_stage['inputLanes']):
                            if pipeline_stage['inputLanes'][index] in stage['outputLanes']:
                                pipeline_stage['inputLanes'].pop(index)
                            else:
                                index += 1

                return self._all_stages.get(stage['stageName'], SchSdcStage)(stage=stage, pipeline=None)
    ...
    def remove_stage(self, label=None, name=None, type=None, library=None):
        ...
        for index, stage in enumerate(self._pipeline[self._config_key]['stages']):
            if stage['stageName'] == stage_instance['stageName']:
                ...
                return self._all_stages.get(stage['stageName'], SchStStage)(stage=stage, pipeline=None)
...
class Pipeline(BaseModel):
    ...
    def configuration(self):
        ...
        """
        definitions = json.loads(self._data['libraryDefinitions'])\
            if 'libraryDefinitions' in self._data and self._data['libraryDefinitions'] else self._library_definitions

        config_definitions = {} if not definitions else definitions['pipeline'][0]['configDefinitions']

        mapping = {}
        for config_definition in config_definitions:
            attribute_name, config_name = get_attribute(config_definition)
            mapping[config_name] = attribute_name

        return LegacyConfiguration(configuration=self._pipeline_definition['configuration'], id_to_remap=mapping)
    ...
    def remove_stages(self, *stages):
        ...
        Args:
            ...
            One or more stage object
        """
        instance_names_to_remove = {stage.instance_name: stage for stage in stages}
        index = 0
        while index < len(self._pipeline_definition_internal['stages']):
            pipeline_stage = self._pipeline_definition_internal['stages'][index]
            if pipeline_stage['instanceName'] in instance_names_to_remove:
                stage_to_remove = instance_names_to_remove[pipeline_stage['instanceName']]
                stage_to_remove.disconnect_input_lanes(all_stages=True)
                stage_to_remove.disconnect_output_lanes(all_stages=True)
                self._pipeline_definition_internal['stages'].pop(index)
            else:
                index += 1
    ...
    def add_stage(self, label=None, name=None, type=None, library=None):
        ...
        try:
            ...
            raise Exception('Editing the pipeline is not supported when the engine is not accessible,'
...
class AzureVMDeployment(Deployment):
    ...
    def is_complete(self):
        """Checks if all required fields are set in this deployment."""
        result = all([self.engine_instances >= 0, self.key_pair_name, self.vm_size, self.zones])
...
class EC2Deployment(Deployment):
    ...
    def is_complete(self):
        ...
        result = all([self.engine_instances >= 0, self.ec2_instance_type, instance_profile])
...
class GCEDeployment(Deployment):
    ...
    def is_complete(self):
        """Checks if all required fields are set in this deployment."""
        return all([self.region, self.zone, self.engine_instances >= 0, self.machine_type, self.instance_service_account])
