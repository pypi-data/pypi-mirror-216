...
class Stage:
    ...
    def __init__(self, stage, label=None):
        ...
        self.configuration = Configuration(self._data['configuration'], id_to_remap=self.__create_id_to_remap__())
    ...
    def __create_id_to_remap__(self):
        ...
        for name, value in self._attributes.items():
            ...
            for config_property in config_properties:
                if isinstance(config_property, StageConfigurationProperty):
                    id_to_remap[config_property.config_name] = name
                    break
        return id_to_remap
    ...
    def disconnect_input_lanes(self, stages=[], all_stages=False):
        ...
        Args:
            stages: A list of :py:class:`streamsets.sdk.sdc_models.Stage` objects to disconnect. Default: `[]`
        ...
        stage_inputs = {}
        for input_lane in self.input_lanes:
            instance_name = input_lane.split('OutputLane')[0]
            if instance_name in stage_inputs:
                stage_inputs[instance_name].append(input_lane)
            else:
                stage_inputs[instance_name] = [input_lane]

        if all_stages:
            stages = self._pipeline.stages

        for stage in stages:
            if stage.instance_name in stage_inputs:
                for input_lane_name in stage_inputs[stage.instance_name]:
                    stage.output_lanes.remove(input_lane_name)
                    self.input_lanes.remove(input_lane_name)

    def disconnect_output_lanes(self, stages=[], all_stages=False):
        ...
        for stage in stages:
            if stage.instance_name == self.instance_name:
                continue

            index = 0
            while index < len(stage.input_lanes):
                input_lane = stage.input_lanes[index]
                if input_lane.split('OutputLane')[0] == self.instance_name:
                    stage.input_lanes.pop(index)
                    self.output_lanes.remove(input_lane)
                else:
                    index += 1
