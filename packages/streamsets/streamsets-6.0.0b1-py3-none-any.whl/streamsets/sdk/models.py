...
class Configuration:
    ...
    def __init__(self, configuration=None, property_key='name', property_value='value', id_to_remap=None, **kwargs):
    ...
    def __getattr__(self, key):
        for config in self._data:
            if config[self.property_key] == key or self._id_to_remap.get(config.get(self.property_key)) == key:
                break
        else:
            raise KeyError(key)
    ...
    def __repr__(self):
        configs = {}
        for config in self._data:
            key = self._id_to_remap.get(config.get(self.property_key)) or config[self.property_key]
            if 'type' not in config:
                value = config['value']
    ...
    def __dir__(self):
        # This is done to try to show the human_readable_name of the attribute if available in _id_to_remap
        # else showcase the attribute as it is
        data_attributes = [self._id_to_remap.get(sub[self.property_key], sub[self.property_key]) for sub in self._data]

        return sorted(list(dir(object)) + list(self.__dict__.keys()) + data_attributes)
