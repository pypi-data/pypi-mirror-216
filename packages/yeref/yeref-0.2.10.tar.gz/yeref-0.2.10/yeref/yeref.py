...
# region payment
async def update_subscribe(bot, BASE_D, MEDIA_D, EXTRA_D, fernet_premium_key, fernet_common_key):
    try:
            ...
            try:
                ...
                if USER_ISPAID == 1 and USER_DTPAID and (dt_ - datetime.datetime.strptime(USER_DTPAID, '%d-%m-%Y_%H-%M-%S')).days > 31:
                    ...
                    if get_chat_member_.status in ['member', 'administrator', 'creator']:
                        ...
                        sql = "SELECT BOT_TID, BOT_USERNAME FROM BOT WHERE OWNER_TID=?"
                        data = await db_select(sql, (USER_TID,), BASE_D)

                        for it in data:
                            BOT_TID, BOT_USERNAME = it[0]
                            await export_template(bot, MEDIA_D, EXTRA_D, USER_TID, USER_LZ,
                                                  BOT_TID, BOT_USERNAME, fernet_premium_key, fernet_common_key)

                            sql = "DELETE FROM BOT WHERE BOT_TID=?"
                            await db_change(sql, (BOT_TID,), BASE_D)

                        if len(data):
                            one_url = donate_bot_rub if USER_LZ == 'ru' else donate_bot_eur
                            text = yeref.l_subscribe_isfree[USER_LZ].format(one_url)
...
async def export_template(bot, MEDIA_D, EXTRA_D, chat_id, lz, BOT_TID, BOT_USERNAME, fernet_premium_key, fernet_common_key, is_premium_owner_=False):
    result = None
    try:
        ...
        if not os.path.exists(BASE_BOT):
        ...
        shutil.copyfile(BASE_BOT, BASE_BOT_COPY)

        # handle copy of base
        sql = "UPDATE TRG SET TRG_USERS=NULL"
        await db_change(sql, (), BASE_BOT_COPY)
        sql = "UPDATE ACT SET ACT_USERS=NULL"
        await db_change(sql, (), BASE_BOT_COPY)
        sql = "UPDATE MSG SET MSG_USERS=NULL"
        await db_change(sql, (), BASE_BOT_COPY)
        sql = "DROP TABLE POST"
        await db_change(sql, (), BASE_BOT_COPY)
        sql = "DROP TABLE USER"
        ...
        sql = "SELECT MSG_VID, MSG_USERS FROM MSG"
        data = await db_select(sql, (), BASE_BOT)
        for item in data:
            GROUP_VID, GROUP_USERS = item
            await update_views(GROUP_VID, 'MSG', [], zip_dir, state_files)

        sql = "SELECT TRG_VID, TRG_USERS FROM TRG"
        data = await db_select(sql, (), BASE_BOT)
        for item in data:
            GROUP_VID, GROUP_USERS = item
            await update_views(GROUP_VID, 'TRG', [], zip_dir, state_files)

        sql = "SELECT ACT_VID, ACT_USERS FROM ACT"
        data = await db_select(sql, (), BASE_BOT)
        for item in data:
            GROUP_VID, GROUP_USERS = item
            await update_views(GROUP_VID, 'ACT', [], zip_dir, state_files)
        ...
        caption = yeref.l_template_is_ready[lz].format(BOT_USERNAME)
    ...
    try:
        for state_file in state_files:
            ...
            if GROUP_VID in tmp_json:
                ...
                if GROUP_VIEW == 'TRG':
                    v_attrs['children'][4]['attrs']['text'] = str(len(GROUP_USERS))
                elif GROUP_VIEW == 'ACT':
                    v_attrs['children'][3]['attrs']['text'] = str(len(GROUP_USERS))
                elif GROUP_VIEW == 'MSG':
                    v_attrs['children'][3]['attrs']['text'] = str(len(GROUP_USERS))
