...
class SailUiForm:
    ...
    @raises_locust_error
    def click_grid_link(self, column_name: str, row_index: int, grid_label: str = None, grid_index: int = None, locust_request_label: str = "") -> 'SailUiForm':
        """
        Click on a link in a grid

        Either a label or an index is required, indices are useful if there is no title for the grid

        Args:
            column_name (str): The name of the column the link is in
            row_index (int): The row in the column to click on
        ...
        """
        grid = self.grid_interactor.find_grid_by_label_or_index(self.state, label=grid_label, index=grid_index)
        ...
        for column in grid["columns"]:
            if column["label"] == column_name:
                ...
                component = row["value"]["values"][0]["values"][0]["link"]
                new_state = self._dispatch_click(component=component, locust_label=locust_request_label)
                reeval_url = self._get_update_url_for_reeval(self.state)
                return self._reconcile_state(new_state, form_url=reeval_url)
        if grid_label:
            raise Exception(f"Column with name {column_name} not found in grid with label {grid_label}")
        else:
            raise Exception(f"Column with name {column_name} not found in grid with index {grid_index}")
