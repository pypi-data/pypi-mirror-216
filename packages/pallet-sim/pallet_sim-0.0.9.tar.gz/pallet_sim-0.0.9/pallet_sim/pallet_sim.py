...
class Pallet_Sim:
    ...
    def show_3D_pallet(self, pallet_index: int) -> None:
        ...
        pallet_visualizer.show_pallet(self.__order.get_pallet(pallet_index), True)
...
def main():
    ...
    if not pallet_sim.load(path_data=rf".\2022-07-27\U1418490.{code}.pal.xml"):
        print("Failed to load order from file")
        return

    # pallet_sim.load_order_from_path("U1418507.0615622.pal.xml")
    result_xml = pallet_sim.run_simulation(view=True, thread_pallets=False, thread_tests=True)
    pretty_print(result_xml, "XML")

    path = rf"obfuscated_output/{code}"
    ...
    pallet_sim.run_simulation(pallet_nr=pallet_sim.get_pallet_nr(1), view=True, color_by="weight", fps=120)
