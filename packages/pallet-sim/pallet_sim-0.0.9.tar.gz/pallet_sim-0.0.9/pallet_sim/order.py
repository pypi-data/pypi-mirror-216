...
def main() -> None:
    ...
    path = r"obfuscated_output/0615622.order.json"
    ...
    model = mujoco.MjModel.from_xml_string(order.get_model_data(model_type="mujoco", pallet_index=0), {
        os.path.basename(asset).lower(): open(f"./textures/{asset}", "rb").read() for asset in
        os.listdir("./textures")
    })
...
def main2():
    # iterate over .pal.xml files in temp_singleLayerHomogen
    for f in os.listdir("temp_singleLayerHomogen"):
        if f.endswith(".pal.xml"):
            order = Order(path_data="temp_singleLayerHomogen/" + f)
            for p in order.iterate_models_data(model_type="mujoco"):
                pass
