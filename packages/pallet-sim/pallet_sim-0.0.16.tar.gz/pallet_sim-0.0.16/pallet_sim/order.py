...
def main() -> None:
    ...
    path = pkg_resources.resource_filename(__name__, r"./examples/0615610.order.json")
    ...
    model = mujoco.MjModel.from_xml_string(order.get_model_data(model_type="mujoco", pallet_index=0), {
        os.path.basename(asset).lower(): open(pkg_resources.resource_filename(__name__, f"./textures/{asset}"), "rb").read() for asset in
