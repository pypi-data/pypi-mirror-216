...
import abc
import asyncio
from typing import Any, Union, List, Type, final
...
from kaiju_tools.types import Namespace, NSKey, Scope, SCOPES
...
class Listener(ContextableService, abc.ABC):
    ...
    async def _process_request(self, data) -> None:
        ...
        body, headers = data
        headers, result = await self._rpc.call(body=body, headers=headers, nowait=True, scope=self._scope)
...
class StreamRPCClient(BaseRPCClient, abc.ABC):
    ...
    async def call(
        ...
        if headers is None:
            headers = {}
        headers['_topic'] = Namespace(env=self.app.env, name=app).create_namespace('_stream').get_key(topic)
