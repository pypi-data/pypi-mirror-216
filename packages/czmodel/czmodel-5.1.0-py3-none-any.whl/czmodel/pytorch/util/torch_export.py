...
def switch_shape(shape: Tuple[int, ...]) -> Tuple[int, ...]:
    ...
    """
    if len(shape) != 3:
        raise ValueError(f"The provided shape ({list(shape)}) should have three dimensions.")
...
def convert_pytorch_to_onnx(
    ...
    """
    model = torch.load(saved_model_dir)
    model.eval()
    if post:
        new_model = nn.Sequential(Permute((0, 3, 1, 2)), model, Permute((0, 2, 3, 1)))
    else:
        new_model = nn.Sequential(Permute((0, 3, 1, 2)), model)
...
class BasePyTorchConverter(BaseConverter[T, "Module"]):
    ...
    @abstractmethod
    def convert_to_onnx(
        self, tmpdir_pytorch: str, model_metadata: T, onnx_path: str, input_shape: Tuple[int, ...]
    ) -> None:
    ...
    def convert(
        ...
        self._validate_metadata(model_metadata, list(switch_shape(input_shape)), list(switch_shape(model_output_shape)))
...
class DefaultPyTorchConverter(BasePyTorchConverter[ModelMetadata]):
    ...
    def convert_to_onnx(
        self, tmpdir_pytorch: str, model_metadata: ModelMetadata, onnx_path: str, input_shape: Tuple[int, ...]
    ) -> None:
        """Abstract method of converting PyTorch model to ONNX.
        ...
        convert_pytorch_to_onnx(
            tmpdir_pytorch, onnx_path, input_shape, model_metadata.model_type in NEED_POSTPROCESSING
        )
...
class LegacyPyTorchConverter(BasePyTorchConverter[LegacyModelMetadata]):
    ...
    def convert_to_onnx(
        self, tmpdir_pytorch: str, model_metadata: LegacyModelMetadata, onnx_path: str, input_shape: Tuple[int, ...]
    ) -> None:
        """Abstract method of converting PyTorch model to ONNX.
