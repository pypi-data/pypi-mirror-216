...
class PyTorchBaseConverter(Generic[T, S]):
    ...
    def convert_from_model_spec(
        ...
        model = model_spec.model if isinstance(model_spec.model, Module) else torch.load(model_spec.model)
...
class LegacyConverter(PyTorchBaseConverter[LegacyModelSpec, LegacyModelMetadata], UnpackModel[LegacyModelMetadata]):
