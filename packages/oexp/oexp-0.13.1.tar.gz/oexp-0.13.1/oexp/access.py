...
JAR_URL = f"https://matt-central.s3.us-east-2.amazonaws.com//0/versioned/front/651/oexp-front-0-all.jar"
...
def choice(value=NO_DEFAULT, image=DEFAULT_VALUE):
    _check_required_parameters(**dict(value=value))
    return Choice(value, image)
...
class RawStimPath(EncryptedTrialStim, KBVClass):
    def __init__(self, *args, _id=None):
        _ensure_synchronized()
        if _id is None:
            jbridge._init_java()
...
def encrypted_stimulus(path=NO_DEFAULT):
    _check_required_parameters(**dict(path=path))
    return EncryptedStimulus(path)


# [[matt.oexp.olang.model.stim.image.EncryptedStimulus]]
class EncryptedStimulus(EncryptedTrialStim, KBClass):
    def __init__(self, *args, _id=None):
        _ensure_synchronized()
        if _id is None:
            jbridge._init_java()
            _sendall(CREATE_OBJECT)
            _send_string("matt.oexp.olang.model.stim.image.EncryptedStimulus")
            _send_long(args[0]._id._id)
            self._id = _object_id(_recv_long())
        else:
            self._id = _object_id(_id)

    # [[matt.oexp.olang.model.stim.image.EncryptedStimulus#path]]
    @property
    def path(self) -> EncryptedStimulusPath:
        _ensure_synchronized()
        _sendall(GET_VAL)
    ...
    # [[matt.oexp.olang.model.stim.image.EncryptedStimulus#url]]
    @property
    def url(self) -> str:
        _ensure_synchronized()
        _sendall(GET_VAL)
