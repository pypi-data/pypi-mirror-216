...
def mbApi(region: Optional[str] = None, source: Optional[str] = None, refreshAuth: bool = True) -> MbApi:
  global __mbApi
  if not isAuthenticated():
    _performLogin(__mbApi, refreshAuth=refreshAuth, region=region, source=source)
  return __mbApi
...
def _performLogin(api: MbApi,
  ...
  setCurrentBranch(_determineCurrentBranch())
...
def _printLoginMessage(api: MbApi, source: Optional[str] = None, displayId: Optional[str] = None):
  if source is None:
    source = "notebook" if inNotebook() else "terminal"
  linkUrl = api.getLoginLink(source)
