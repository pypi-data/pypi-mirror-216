...
def SAR_Raster_Correction(input_path, output_path, limit=None, median_kernel_size=5,
                      bilateral_win_size=3, bilateral_sigma_color=0.05, bilateral_sigma_spatial=3,
                      clip_percentiles=[2, 98], optical=False):
    """
    Apply raster correction to a set of raster images.

    Args:
        input_path (str): Path to the input image directory.
        output_path (str): Path to the output directory.
        limit (int): Maximum number of images to process. Set to None for all images.
        median_kernel_size (int): Size of the median filter kernel.
        bilateral_win_size (int): Size of the bilateral filter window.
        bilateral_sigma_color (float): Sigma value for the bilateral filter color space.
        bilateral_sigma_spatial (int): Sigma value for the bilateral filter spatial space.
        clip_percentiles (list): List of percentiles used for clipping the data.
        optical (bool): Recommended for optical imagery

    Returns:
        Corrected rasters for further processing
    """
    ...
    if limit is not None:
        ...
        image_files = [file for file in os.listdir(input_path) if file.endswith('.tif')][:limit]
    else:
        image_files = [file for file in os.listdir(input_path) if file.endswith('.tif')]
    ...
    for image_file in image_files:
        try:
            ...
            axes[0, 2].set_title('median_filter')
            ...
            axes[1, 2].set_title('median_filter Histogram')
