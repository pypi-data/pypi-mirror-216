...
class DerechosAutoriaWizard(models.TransientModel):
    ...
    def generar_derechos_autoria(self):
        domain = [
            ('categ_id', 'in', [5, 11]) # Todos los productos con categoría "All / Libros" (con id 5) o "All / Libro Digital" (con id 11)
        ]
        libros = self.env['product.template'].search(domain)
        ...
        for libro in libros:
            ...
            if not ddaa:
                ddaa = self.env['product.template'].create({
                    ...
                    'receptora_derecho_autoria': libro.author_name,
