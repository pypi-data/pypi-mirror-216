...
If the AI does not know the answer to a question, it truthfully says it does not know. Potentially relevant
information is given below. The AI only uses this information if it is needed to respond to the final user
...
The current conversation is below. The AI may or may not use the above context to complete its response.
Conversation:
{input}
ASSISTANT:"""
...
class LlamaCppChat(Service):
    ...
    def onPrompt(self, prompt: str):
        self.worker_queue.put(prompt)
...
class Reranker(Service):
    def __init__(self, name):
        super(Reranker, self).__init__(name)
        self.cross_encoder = CrossEncoder('cross-encoder/ms-marco-TinyBERT-L-2')
        self.top_k = 5

    def publishRerankings(self, results: List[ChatMessage]):
        return results

    def rerank(self, query: str, passages: List[ChatMessage]):
        model_inputs = [[query, passage.message] for passage in passages]
        scores = self.cross_encoder.predict(model_inputs)
        # Sort the scores in decreasing order
        results = [{'input': inp, 'score': score} for inp, score in zip(passages, scores)]
        results = sorted(results, key=lambda x: x['score'], reverse=True)
        self.invoke("publishRerankings", [result["input"] for result in results[:self.top_k]])
...
def connection_done():
    ...
    recall_reranker = Reranker("recall_reranker")
    reranker_pair = PairAssembler("reranker_pair")
    ...
    #       |                     +-----> input_mapping_pair ---------------------------------------------+
    #       |                                                                                             +--> prompt_input_mapper ----+
    #       +-> recall_embeddings --> solr.recallMemories()---> recall_reranker --> recall_mapping_pair --+                            |
    #       |                                                    ^                                                                     |
    #       +----------------------------------------------------+                                                                     |
    ...
    reranker_pair.subscribe(in_loop.getFullName(), "publishText", callback_method="setFirst")
    reranker_pair.subscribe(solr.getFullName(), "recallMemories", callback_method="setSecond")
    recall_reranker.subscribe(reranker_pair.getFullName(), "publishPair", callback_method="rerank")

    recall_mapping_pair.reset_first = False
    recall_mapping_pair.setFirst("context")
    recall_mapping_pair.subscribe(recall_reranker.getFullName(), "publishRerankings", recall_mapping_pair.getFullName(),
                                  "setSecond")
