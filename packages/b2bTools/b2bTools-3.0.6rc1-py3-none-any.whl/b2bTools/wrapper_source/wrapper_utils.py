...
class SingleSeq:
    ...
    def get_metadata(self, output_filepath, sep=","):
        execution_time_df = self._get_execution_time_df()
        statistics_df = self._get_statistics_df()
    ...
    def _get_statistics_df(self):
        ...
        for sequence_id, predictions in self.get_all_predictions().items():
            print("Getting statistics from: ", sequence_id)
            statistics_data = {}
    ...
    def _get_execution_time_df(self):
        stats_metadata_list = []
...
class MultipleSeq:
    ...
    def get_all_predictions_tabular(self, output_filepath, sep=","):
        ...
        rows_list = []
        all_predictions =  self.get_all_predictions_msa()
    ...
    def _get_execution_time_df(self):
        ...
        # NOTES (Sophie & Adrián): Room for improvements
        # Pandas's read_json: https://pandas.pydata.org/pandas-docs/version/1.1/reference/api/pandas.read_json.html
        for sequence_id, predictions in self.get_all_predictions_msa().items():
            if sequence_id == "sequence":
                continue
