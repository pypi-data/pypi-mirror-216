...
def get_bulk_write_list(data_list: list, field_list: list):
    """
    获取批量插入的数据对象列表
    :param self:
    ...
    for data_json in data_list:
        filter_data = get_filter_data(field_list, data_json)
        if not filter_data:
            continue
...
def get_filter_data(field_list: list, data_json: dict):
    """
