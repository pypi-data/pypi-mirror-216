...
def _expand_positional_references(
    scope: Scope, expressions: t.Iterable[E]
) -> t.List[E]:
    new_nodes = []
...
def _qualify_columns(
    scope: Scope, resolver: Resolver, strict_schema_adherence=True
) -> None:
...
def _expand_stars(
    ...
    if pivot and has_pivoted_source:
        ...
        pivot_output_columns = [
            col.output_name for col in pivot.args.get("columns", [])
        ]
    ...
    for expression in scope.selects:
        ...
        for table in tables:
            ...
            if columns and "*" not in columns:
                if (
                    pivot
                    and has_pivoted_source
                    and pivot_columns
                    and pivot_output_columns
                ):
                    implicit_columns = [
                        col for col in columns if col not in pivot_columns
                    ]
                    new_selections.extend(
                        exp.alias_(
                            exp.column(name, table=pivot.alias), name, copy=False
                        )
                ...
                for name in columns:
                    if (
                        name in using_column_tables
                        and table in using_column_tables[name]
                    ):
                        ...
                        new_selections.append(
                            alias(
                                exp.Coalesce(
                                    this=coalesce[0], expressions=coalesce[1:]
                                ),
                                alias=name,
                                copy=False,
                            )
                        ...
                        new_selections.append(
                            alias(column, alias_, copy=False)
                            if alias_ != name
                            else column
                        )
            else:
        ...
        if self._source_columns is None:
            self._source_columns = {
                ...
                for k in itertools.chain(
                    self.scope.selected_sources, self.scope.lateral_sources
                )
            }
