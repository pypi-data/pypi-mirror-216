...
class LocationCode:
    ...
    def get_level_code(
        ...
        * The provided `components` must have
        ...
        level_codes = dict(zip(self.avail_levels, [None] * len(self.avail_levels)))
...
def generate_loc_code(data: pd.DataFrame, best_lvl_cols: List[str]) -> pd.DataFrame:
    """
    ...
    * `level_3_code`: code for ward
    ...
    for level in AVAIL_LEVELS:
        generated_data[f"level_{level}_code"] = [code[level] for code in row_codes]
