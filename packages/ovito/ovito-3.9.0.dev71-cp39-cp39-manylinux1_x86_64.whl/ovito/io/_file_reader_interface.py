...
import os
import traits.api
from typing import Any, Optional, Generator, Union, Callable
...
class FileReaderInterface(traits.api.HasStrictTraits):
    ...
    if os.environ.get('OVITO_SPHINX_BUILD', False):
        ...
        @abc.abstractmethod
        def scan(self, filename: str, register_frame: Callable):
            """
            ...
            The *frame_info* value and the *label* text describe one trajectory frame discovered by your scan method
            ...
            A typical implementation of the :py:meth:`scan` method may look as follows for a file format that requires
            scanning through the contents of the file to discover the byte offsets at which trajectory frames begin::

                def scan(self, filename, register_frame):

                    with open(filname, "r") as fin:
                        ...
                        while fin:
                            # Register the frame that begins at the current byte offset:
                            offset = fin.tell()
                            register_frame(frame_info=offset)

                            # For progress reporting in the OVITO GUI:
                            yield offset / file_size

                            # Skip to beginning of next frame in file:
                            ...
