...
def transfer_products(conn, owner="Administrator"):
    print("同步产品表数据。。。")
    st = time.perf_counter()
...
def transfer_calendar(conn, owner="Administrator"):
    print("同步设备日历数据。。。")
    st = time.perf_counter()
    ...
    data = select_data_from_table(
        conn,
        table_item,
        columns=["parent", "start_date", "end_date"],
        where_clause=f"owner = %s",
        where_params=(owner,),
    )
    results = []
    for item in data:
        for date in pd.date_range(item["start_date"], item["end_date"]):
            results.append({"machine_id": item["item_code"], "date": date.strftime("%Y-%m-%d"), "shift": item["shift"]})
    # filename = os.path.join(get_project_root(), "output", "product.csv")
    df_bom = pd.DataFrame(results)
    # df_bom.to_csv(filename, index=False)
...
def transfer_shift(conn, owner="Administrator"):
    print("同步班制数据。。。")
    st = time.perf_counter()
    ...
    data = select_data_from_table(
        conn,
        table_item,
        columns=["shift", "start_time", "end_time"],
        where_clause=f"owner = %s",
        where_params=(owner,),
    )
    results = []
    for item in data:
        results.append({"shift": item["shift"], "start_time": item["start_time"], "end_time": item["end_time"]})
    # filename = os.path.join(get_project_root(), "output", "product.csv")
    df_bom = pd.DataFrame(results)
    # df_bom.to_csv(filename, index=False)
...
def get_data(owner, need_data_list: List[str]):
    """
    :param owner：用户邮箱
