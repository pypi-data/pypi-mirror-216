...
def initialize_folder():
    ...
    trtweightsPath = OCRHomePath + "/trt_model"
    ...
    if not os.path.exists(trtweightsPath):
        os.makedirs(trtweightsPath, exist_ok=True)
...
def save_dir(url, model_name, onnx_mode=True):
    url_ = url
    initialize_folder()
    home = get_model_home()
...
def get_model(url, onnx_mode=True, front_side=False, back_side=False, crop_nid=False, classify=False,
              ...
              mrc_crop=False, mrc_detect=False, encoder=False, decoder=False, encoder_trt=False, decoder_trt=False):
    model_mapping = {
