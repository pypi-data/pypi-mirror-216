...
class TableMetaParams:
    ...
    use_lightweight_deletes: bool = False
...
class ClickhouseCubeBackend(CubeBackend):
    ...
    def delete_records(self, query: CubeQuery) -> None:
        ...
        if meta.use_lightweight_deletes:
            # without any filter, the WHERE part is still required, so we use 1 as a dummy condition
            whole_text = f"DELETE FROM {table} WHERE {where or 1}"
        else:
