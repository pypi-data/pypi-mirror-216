...
class TestJobExecutor(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.job_executor_producer = JobExecutor(redis_client=redis_client, redis_config=redis_config, consumer=False, import_workers=0, query_workers=0, export_workers=0, sink_workers=0, branching_workers=0)
        self.job_executor_consumer = JobExecutor(redis_client=redis_client, redis_config=redis_config, consumer=True, import_workers=1, query_workers=1, export_workers=1, sink_workers=1, branching_workers=1)
    ...
    async def test_discover_new_import_query_and_export_queue(self):
        ...
        job5 = FakeTestJob(JobKind.SINK, self.user)
