...
class CLIGitRelease:
    ...
    def diff(self, current_ws_commit: str) -> List[Diff]:
        ...
        return [diff for diff in self.head().commit.diff(current_ws_commit) if diff.change_type in ['A', 'D', 'R', 'M']]

    def validate_local_for_release(self, current_release: Dict[str, Any], check_dirty: bool = True, check_outdated: bool = True, check_new: bool = True):
        if self.is_dirty_to_release() and check_dirty:
            raise CLIGitReleaseException(FeedbackManager.error_commit_changes_to_release())
    ...
    def get_changes_from_current_release(self, current_release: Dict[str, Any], filenames: List[str]):
        ...
        for diff in diffs:
            if diff.a_path and 'vendor/' not in diff.a_path:
...
async def wait_job(tb_client, job_id, job_url, label, timeout=None, wait_observer=None, semver=None):
...
async def diff_command(filenames: Optional[List[str]], fmt: bool, client: TinyB, no_color: Optional[bool] = False, with_print: Optional[bool] = True,
                       verbose: Optional[bool] = None, clean_up: Optional[bool] = False):
