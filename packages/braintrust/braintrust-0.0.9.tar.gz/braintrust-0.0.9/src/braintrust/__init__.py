...
class ScoreSummary(SerializableDataClass):
    ...
    def __str__(self):
        ...
        diff_pct = abs(self.diff) * 100

        # pad diff_pct so that it's exactly 2 characters then decimal then 2 decimals
        diff_pct = f"{diff_pct:05.2f}"
        ...
        return textwrap.dedent(
            f"""{diff_score} {score_name} score\t({self.improvements} improvements, {self.regressions} regressions)"""
        )
