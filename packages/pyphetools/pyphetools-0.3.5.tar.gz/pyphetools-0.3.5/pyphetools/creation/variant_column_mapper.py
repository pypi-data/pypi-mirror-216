...
class VariantColumnMapper:
    ...
    def map_cell(self, cell_contents, genotype_contents=None, delimiter=None) -> List[Variant]:
        ...
        for item in items:
            if item in self._non_hgvs_variant_map:
                varMapper: VarMapper = self._non_hgvs_variant_map.get(item)
                interpretation = varMapper.to_ga4gh_variant_interpretation()
    ...
    def variant_interpretation_to_string(self, vinterpretation):
        ...
        for exprss in vinterpretation.expressions:
            if exprss.syntax == "hgvs.c":
                return exprss.value
        # then check for a description
        if vinterpretation.variation_descriptor is not None:
            ...
            # should never happen!
            raise ValueError(f"Could not create string representation of vinterpretation")
