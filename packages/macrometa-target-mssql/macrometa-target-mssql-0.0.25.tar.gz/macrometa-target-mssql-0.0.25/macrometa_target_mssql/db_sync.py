...
class DbSync:
    ...
    def query(self, query, params=None, require_database=True, ignore_rowcount=False):
    ...
    def bulk_insert_records(
        ...
        self.logger.info(f"recordss11: {records.values()}")
        first_record = next(iter(records.values()))
        keys = first_record.keys()

        # Define the SQL INSERT statement
        insert_query = "INSERT INTO {} ({}) VALUES ({})".format(full_table_name, ', '.join(keys), ', '.join(['%s'] * len(keys)))
        self.logger.info("Inserting with SQL: %s", insert_query)

        self.logger.info(f"recordss222: {records.values()}")
        ...
        for record in records.values():
            ...
            for key in keys:
                if key in record:
                    ...
                    record_values.append(None)  # Handle missing keys gracefully by inserting None
    ...
    def to_sql_type(self, jsonschema_type: dict) -> sqlalchemy.types.TypeEngine:  # noqa
        ...
        if self._jsonschema_type_check(jsonschema_type, ("number",)):
            if self.connection_config.get("prefer_float_over_numeric", False):
        ...
        if self._jsonschema_type_check(jsonschema_type, ("object",)):
            return cast(sqlalchemy.types.TypeEngine, sqlalchemy.types.VARCHAR(None))
    ...
    def process_batch(self, records) -> None:
        ...
        with self.open_connection(require_database=True) as connection:
            with connection.cursor(as_dict=True) as cur:
                if key_properties:
