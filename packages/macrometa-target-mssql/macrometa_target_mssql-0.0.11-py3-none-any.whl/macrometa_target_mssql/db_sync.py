    ...
    def query(self, query, params=None, require_database=True):
        ...
        with self.open_connection(require_database=require_database) as connection:
            with connection.cursor(as_dict=True) as cur:
                ...
                self.logger.info(f"cursor1:{cur}")
                result = cur.fetchall()
                self.logger.info(f"result1:{result}")
                cur.execute(
                    query
                )
                self.logger.info(f"cursor2:{cur}")
                if cur.rowcount > 0:
                    self.logger.info(f"rowcount2:{cur.rowcount}")
                result = cur.fetchall()
                self.logger.info(f"result2:{result}")
                if cur.rowcount > 0:
                    self.logger.info(f"rowcount3:{cur.rowcount}")
                ...
                if cur.rowcount > 0:
                    self.logger.info(f"rowcount4:{cur.rowcount}")
                    return cur.fetchall()
                self.logger.info("reached exit...")
                return []
