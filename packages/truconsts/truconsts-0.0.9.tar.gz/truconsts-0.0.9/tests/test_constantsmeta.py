...
class Constants(BaseConstants):
    ...
    # Call it once and never mutate it, Cache[Immutable] is same as
    # Immutable[Cache]
    LAZY_IMMUTABLE_STR: Cache[Immutable[str]] = get_root_dir
    NUMBER: Immutable[int] = 123456


class ConstantsWithoutAnnotations(
        BaseConstants):  # behaves like your regular class
    ROOT_DIR_ = 'HELLO'
    NEW_ = 'HIHI'
    OLD_ = 'OLD'
...
def test_subclass_instance_has_no_dict_two():
    # refresh the class to avoid RuntimeError: cannot reuse already awaited
    # coroutine
    class AsyncConstants(BaseConstants):
        ASYNC_STR: Yield[str] = get_yield_root_dir
        ASYNC_IMMU_STR: Yield[Immutable[str]] = another_coroutine
