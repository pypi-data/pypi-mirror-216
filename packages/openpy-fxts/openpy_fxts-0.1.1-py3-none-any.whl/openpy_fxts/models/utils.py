...
def _values_preliminary(model, dict_test, config, name_model=None):
...
def _values_preliminary_mlp(model, dict_test, config, name_model=None):
    X_test = dict_test['X']
...
def dict_metrics(y_test_inverse, yhat_inverse, config, name_model):
    ...
    df_ytest = pd.DataFrame(ytest_2d, columns=config['y_colname'])
    df_yhat = pd.DataFrame(yhat_2d, columns=config['y_colname'])
    scorer_dict = {}
    for idx, y_colname in enumerate(config['y_colname']):
