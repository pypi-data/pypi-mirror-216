        ...
        **kwargs,
        ...
        if is_not_cluster_compatible:
            ...
            if s3_export_buckets or s3_import_buckets:
                ...
                s3_import_policy = self._instance.node.find_child("S3ImportRole").node.find_child("DefaultPolicy").node.default_child
                rds_s3_list_statement = _iam.PolicyStatement(
                    actions=[ "s3:ListAllMyBuckets" ],
                    resources=[ "*" ],
                )
                s3_import_policy.policy_document.add_statements(rds_s3_list_statement)
