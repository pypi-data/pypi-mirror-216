...
def standardize_transfer_syntax(dcm: DICOM) -> str:  # pragma: no cover
    ...
    if found_ts == standard:
        ...
        if found_ts.is_compressed and "PixelData" in dcm:
            ...
            # https://pydicom.github.io/pydicom/dev/reference/generated/pydicom.config.pixel_data_handlers.html
            # The first handler that both announces that it supports the
            # transfer syntax and does not raise an exception, either in getting
            # the data or when the data is reshaped to the correct dimensions,
            # is the handler that will provide the data.
