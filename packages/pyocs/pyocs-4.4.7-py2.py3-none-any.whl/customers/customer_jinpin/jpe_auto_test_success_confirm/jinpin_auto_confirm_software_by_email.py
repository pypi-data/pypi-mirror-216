...
import My_pyocs_fun
import CheckCustomerConfirmExcelEmail
...
class Logger(object):
    def __init__(self, filename='auto_Confirmdailylog.log', stream=sys.stdout):
...
check_have_confirm_file_path=dirPath+'/customers/customer_jinpin/test_success_confirm/'
fr =open(save_confirm_file_name, 'r',encoding='utf-8')
new_confirm_excel_name = str(fr.read())
fr.close()    
execl_path=dirPath+'/customers/customer_jinpin/test_success_confirm/confirm_excel/'+new_confirm_excel_name
          
Workbook = load_workbook(execl_path)
# 获取行列等单元值
#sheet要从获取活动的改为获取未做过的sheet
sheet = Workbook.active
max_rows_new = sheet.max_row

#to_engyneer = ['linxiangna@cvte.com','chenchaoxiong@cvte.com']
...
def write_filename_have_confirm(confirm_Attachments):
    # Workbook属于创建才用
    ...
    path = check_have_confirm_file_path+'/已提交确认的表格.xlsx'
    ...
    while row_index <= max_rows_new:
        ...
        if None != b.value:
            if confirm_Attachments == b.value:
                return False

    row = [confirm_Attachments]
    print(row)
    sheet.append(row)
    writebook.save(path)
    return True
...
#print("check flag",write_filename_have_confirm(new_confirm_excel_name) )
if write_filename_have_confirm(new_confirm_excel_name) == True :
    #gl.set_recive_new_email_flag(False)
    #print("auto run",Check_in_confirm.recive_new_email_flag)   

    replaceText = "_"
    be_replaceText = "-"
    while row_index <= max_rows_new:
        soft_version = str(sheet.cell(row=row_index, column=6).value)
        #print("str soft_version",soft_version)
        sheet.cell(row=row_index, column=6).value = soft_version.replace(be_replaceText, replaceText, 1)
        #print("soft_version",str(sheet.cell(row=row_index, column=6).value))
        row_index += 1
    ...
    while row_index <= max_rows_new:
        ...
        soft_version = sheet.cell(row=row_index, column=6)
    ...
    whole_text = confirm_success_list+text + text_chenfang + text_chenchaoxiong + text_xiangna + text_honghai + suggest_info_text + suggest_to_test_text
    print(whole_text)
