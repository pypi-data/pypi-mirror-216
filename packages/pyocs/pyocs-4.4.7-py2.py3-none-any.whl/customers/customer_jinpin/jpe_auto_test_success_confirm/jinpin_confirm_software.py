...
# 打开Excel文件读取数据
dirPath = os.getcwd()
execl_path=dirPath+'/jpe_auto_test_success_confirm/客户回复确认表格.xlsx'             
...
for key in order_soft_success_list:
    ...
    if((str(key[0]) !=None) and  (str(key[1]) !=None)):  
        ...
        ocs_number = confirm_task.get_ocs_number_from_sw(sw_name, order_info)
        print("can find  ocs_number",ocs_number)
        if ocs_number is None:
            pass
        else:
            sw_list = confirm_task.get_enable_software_list(ocs_number=ocs_number)
...
for key in upload_soft_to_ocs_success_list:                           
    ...
    if ("chenjiayi8469@cvte.com" == engineer_name):
        chenjiayi8469_upload_ocs_sucess_order.append(tmp_fail_list)
...
for key in order_soft_fail_list:
    ...
    if ("chenjiayi8469@cvte.com" == engineer_name):
        chenjiayi8469_unpass_order.append(tmp_fail_list)
"""
...
new_str_chenjiayi8469 = send_email.failed_list_to_str(chenjiayi8469_unpass_order)
...
upload_ocs_sucess_str_honghai = send_email.failed_list_to_str(chenjiayi8469_upload_ocs_sucess_order)
...
upload_ocs_sucess_text_honghai = text_Enter + "嘉艺:" + text_Enter + upload_ocs_sucess_str_honghai
confirm_success_list = confirm_success_text+upload_ocs_sucess_text_honghai + upload_ocs_sucess_text_chenchaoxiong + upload_ocs_sucess_text_xiangna
...
text_chenjiayi8469 = text_Enter + "嘉艺:" + text_Enter + new_str_chenjiayi8469
...
whole_text = confirm_success_list+text + text_chenjiayi8469 + text_chenchaoxiong + text_xiangna  + suggest_info_text + suggest_to_test_text
print(whole_text)
