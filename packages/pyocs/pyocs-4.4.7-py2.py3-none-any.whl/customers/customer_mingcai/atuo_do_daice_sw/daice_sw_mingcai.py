...
#用vscode运行时，用这个
dirPath = os.getcwd()+'/customers/customer_mingcai/atuo_do_daice_sw'
#window下运行的
#dirPath = os.getcwd()
...
class atuo_do_daice_sw:
    ...
    def check_if_reuse_old(self,new_ocs_info_list,count_nums):
        ...
        for i in range(0,count_nums):
            dst_ocs_number = str(new_ocs_info_list[i][1])
            ...
            dst_charging_items = pd_dst.get_other_app_software()            
            ...
            ocs_subjest_info  = new_ocs_info_list[i][2]
            ...
            #string_liaohao = string_liaohao.strip()
            ...
            if(new_ocs_info_list[i][0] != "MT7601") and (new_ocs_info_list[i][0] != "6M60") and ("MT9602" not in new_ocs_info_list[i][0]) :
                for j in range(count_nums-1,-1,-1):
                    print("第几轮引用j:",j)
                    #现在是找ocs上有未被禁用软件的订单

                    if ocs_list_liaohao[j][3] != "软件审核不通过" and ocs_list_liaohao[j][3] != "软件测试不通过" and ocs_list_liaohao[j][3] != "待录入需求":
                        #if do_daice_task.get_enable_software_can_be_reuse_with_ocs(ocs_list_liaohao[j][0]) == None:
                        if sw.get_enable_software_list(ocs_list_liaohao[j][1]) == None:                        
                            print("get_enable_software_can_be_reuse_with_ocs NONE")
                            continue
                        else:
                            print("ocs_list_liaohao[j][1]",ocs_list_liaohao[j][1])

                            old_backup_ocs_number = str(ocs_list_liaohao[j][1])
                            ...
                            old_charging_items = pd_old.get_other_app_software()                            
 
                            if  (dst_product_name == old_product_name) and (dst_pwm == old_pwm) and (dst_port_info == old_port_info)and (dst_charging_items == old_charging_items):
                                old_ocs_number= ocs_list_liaohao[j][1]
                                break
...
class Logger(object):
    def __init__(self, filename='mingcai_daice_log.log', stream=sys.stdout):
...
new_order_advanced_search = {
    "0":{"search_field_name":"Task.account_id","search_field_id":"560","search_field_type":"19","search_field_rel_obj":"Accounts","search_opr":"TDD_OPER_INC","input1":"\u660e\u5f69","input2":"null","offset":"null"},
...
logging.basicConfig(filename=os.path.join(os.getcwd()+'/customers/customer_mingcai/atuo_do_daice_sw/','mingcai_daice_log.txt'),level=logging.DEBUG)
