    ...
    with open("login.txt", 'r') as f:
        for line in f.readlines():
            if "user=" in line:
                ...
                usermessage="".join(re.findall(p1, strlist[1]))

            elif "password=" in line:
                strlist=line.split("password=")
                passwordmessage="".join(re.findall(p1, strlist[1]))

            elif "path=" in line:
                strlist=line.split("path=")
                pathmessage="".join(re.findall(p1, strlist[1]))

            elif "openbrowser=" in line:
                strlist=line.split("openbrowser=")
                openbrowser="".join(re.findall(p1, strlist[1]))
                openbrowser=int(openbrowser)

        f.close()
    print(pathmessage)
    td = HiMailDownload()
    td.download_file(usermessage, passwordmessage, pathmessage, openbrowser)
