...
order_early_warning_url="http://ocs-api.gz.cvte.cn/tv/TaskRelForecastWarms/index/page:1/tab:status_10/range:all"
faenamexls_name="工程师姓名部门维护表.xls"
home_dir = os.environ['HOME']
workspace = home_dir + '/'
workfilepath=workspace+faenamexls_name
waring_level_path='./waring_level.txt'
ocs_list=[]
engineer_ocs_dict = {}
...
def get_pages_num():
    response = PyocsRequest().pyocs_request_get(order_early_warning_url)
...
def get_all_early_warning_order_list():
    ...
    for p in range(1,pagesnum+1):
        other_page_url = order_early_warning_url.replace("page:1","page:"+str(p))
...
def get_all_name_ocs_dict():
    ...
    for ocsnum in ocs_list:
        ...
        print(sw_engineer)
        cvte_name = get_fae_name_from_drive_xls(sw_engineer)
        print(cvte_name)

        #将姓名和ocs号追加形成字典
        if cvte_name in engineer_ocs_dict:
            if cvte_name == "NO_THIS_NAME":
                tempvalue = engineer_ocs_dict[cvte_name]+","+sw_engineer
            else:
                tempvalue = engineer_ocs_dict[cvte_name]+","+ocsnum
            engineer_ocs_dict.update({""+cvte_name: ""+tempvalue})
        else:
            if cvte_name == "NO_THIS_NAME":
                engineer_ocs_dict.update({""+cvte_name: ""+sw_engineer})
            else:
                engineer_ocs_dict.update({""+cvte_name: ""+ocsnum})
...
def deal_early_warning_tips(waring_level):
    ...
    for key_name in engineer_ocs_dict:
        if key_name == "NO_THIS_NAME":
            ...
            if waring_level == 1:
                msg = "【订单预警系统】 今日您有订单预警需要及时处理,OCS:"+engineer_ocs_dict[key_name]+" \
                \n请登录 https://ocs-api.gz.cvte.cn/tv/TaskRelForecastWarms/index/tab:status_10/range:my 处理!"
            elif waring_level == 2:
                msg = "【订单预警系统】 今日的预警即将处理不及时,请11点半前处理完毕,OCS:"+engineer_ocs_dict[key_name]+" \
                \n请登录 https://ocs-api.gz.cvte.cn/tv/TaskRelForecastWarms/index/tab:status_10/range:my 处理!"            
            elif waring_level == 3:
                msg = "【订单预警系统】 今日的预警处理不及时，要求11.30前完成，请立即处理，谢谢！,OCS:"+engineer_ocs_dict[key_name]+" \
                \n请登录 https://ocs-api.gz.cvte.cn/tv/TaskRelForecastWarms/index/tab:status_10/range:my 处理!"
            else:
                msg = "【订单预警系统】 今日您有订单预警需要及时处理,OCS:"+engineer_ocs_dict[key_name]+" \
                \n请登录 https://ocs-api.gz.cvte.cn/tv/TaskRelForecastWarms/index/tab:status_10/range:my 处理!"                
