...
class DatetimeParser():
    ...
    def _remove_timezone_abbrev(self, dtSeries: pd.Series) -> pd.Series:
        ...
        # Use the first timestamp to check if time zone abbrevation exist
        matchedTzAbbrev = None
        ...
        for tzAbbrev in sortedTzAbbrevs:
            if tzAbbrev in timestampString:
                matchedTzAbbrev = tzAbbrev
                logger.info(f'Found tz abbrev in timestamp series = {matchedTzAbbrev}')
                break

        # Return the original series if no time zone abbreviation is found
        if matchedTzAbbrev is None:
            return dtSeries

        else:

            # Remove the tz abbreviation if it is found
            for index, value in dtSeries.iteritems():
                if matchedTzAbbrev in value:
                    dtSeries[index] = value.replace(matchedTzAbbrev, '').strip()

                else:
                    # Find if other tz abbrevation exists and remove it if found
                    for tzAbbrev in sortedTzAbbrevs:
                        if tzAbbrev in value:
                            logger.info(f'Found a different tz abbrev in the same timestamp series = {tzAbbrev}')
                            dtSeries[index] = value.replace(tzAbbrev, '').strip()
                            break
