...
class IPCCGlossary:
    ...
    def create_annotated_glossary(cls, glossary_html_file, style_class, link_class, write_glossary=True):
        ...
        if write_glossary:
            HtmlLib.write_html_file(glossary.glossary_elem, Path(Path(glossary_html_file).parent, "annotated_glossary.html"))
        return glossary
    ...
    def annotate_lead_entries(self, style_class, use_bold=False, debug=False):
        debug=True
        if use_bold:
            self.div_entries = [div for div in self.glossary_elem.xpath(f".//div[span]") if HtmlStyle.is_bold(div.xpath('./span')[0])]
        else:
