...
class TestBuilderFiQuS(unittest.TestCase):
    ...
    def test_BuilderFiQuS_attributes(self):
        # arrange
        magnet_names = ['MQXA', 'MCBRD']
        append_to_magnet_name = ''
        software = 'FiQuS'
        for magnet_name in magnet_names:
            ...
            if model_data.GeneralParameters.magnet_type == 'multipole':
                ...
                pR = ParserRoxie()
                roxie_data = pR.getData(dir_data=path_data, dir_cadata=path_cadata, dir_iron=path_iron)
            else:
            ...
            self.assertTrue(bF.data_FiQuS.magnet.type in ['multipole', 'CCT_straight'])      # check if magnet type has been set_up to one of the currently supported types
