...
class AnalysisSTEAM:
    ...
    def step_postprocess(self, step, verbose: bool = False): # run sequence variable
        ...
        if step.sources is not None:
            ...
            for source, source_sim_nbr, fiqus_csv_idx in zip(step.sources, step.sources_sim_nrs, step.sources_FiQuS_csv_idx):
                ...
                if source and 'sigma' in source.lower():
                    fiqus_csv_idx = fiqus_csv_idx[0]
                    # Warning when fiqus_csv_idx is not None for other tools than FiQuS
                    ...
                    path_result_txt_Bx = os.path.join(self.settings.local_SIGMA_folder, step.simulation_name, f"{step.simulation_name}_{source_sim_nbr}","output", "mf.Bx.txt")
                    path_result_txt_By = os.path.join(self.settings.local_SIGMA_folder, step.simulation_name, f"{step.simulation_name}_{source_sim_nbr}","output", "mf.By.txt")
                    path_new_file = os.path.join(self.settings.local_SIGMA_folder, step.simulation_name, f"{step.simulation_name}_{source_sim_nbr}","output", "B_field_map2d.map2d")
                    path_reference_roxie = os.path.join(self.settings.local_SIGMA_folder, step.simulation_name, f"{step.simulation_name}_{source_sim_nbr}", "sources", f"{step.simulation_name}_{source_sim_nbr}_ROXIE_REFERENCE.map2d")
                ...
                elif source and 'fiqus' in source.lower():
                    fiqus_csv_idx = fiqus_csv_idx[0]
                ...
                elif source and 'roxie' in source.lower():
                    # Take latest run by reading file.
                    fiqus_csv_idx = fiqus_csv_idx[0]
                    if fiqus_csv_idx is not None:
                        warnings.warn(
                            f"Fiqus csv index is set to other than None for {source}. Observe this is only applicable to FiQuS")
                    path_new_file = os.path.join(self.library_path, 'magnets',  step.simulation_name, 'input', step.simulation_name+".map2d")
            ...
            exported_stats = generate_report_from_map2d(step.path_to_saved_files,
    ...
    def steam_analyze_lhc_event(self, input_csv_file: str, local_PSPICE_folder: str, flag_run_PSPICE: bool):
        circuit_name = self.__get_first_circuit_name(os.path.join(os.getcwd(), input_csv_file))
        if circuit_name.startswith("RCD"):
            self.analyze_RCD_RCO_event(os.path.join(os.getcwd(), input_csv_file), local_PSPICE_folder, flag_run_PSPICE)
        elif circuit_name.startswith(("RD1", "RD2", "RD3", "RD4")):
            circuit_type = "IPD"
            self.analyze_circuit_event(os.path.join(os.getcwd(), input_csv_file), local_PSPICE_folder, circuit_type, flag_run_PSPICE)
        elif circuit_name.startswith(("RQ4", "RQ5", "RQ7", "RQ8", "RQ9", "RQ10")) or (circuit_name.startswith("RQ6.") and len(circuit_name) == 6):
            circuit_type = self.__find_circuit_model(os.path.abspath(os.path.join(os.getcwd(), f"../../../steam_models/circuits/circuit_parameters/IPQ_circuit_parameters.csv")), input_csv_file.split("_")[0].split("\\")[-1])
            self.analyze_circuit_event(os.path.join(os.getcwd(), input_csv_file), local_PSPICE_folder, circuit_type, flag_run_PSPICE)
        else:
            circuit_type = self.__get_circuit_type(circuit_name)
            self.analyze_circuit_event(os.path.join(os.getcwd(), input_csv_file), local_PSPICE_folder, circuit_type, flag_run_PSPICE)
    ...
    def analyze_circuit_event(self, input_csv_file: str, local_PSPICE_folder: str, circuit_type: str, flag_run_PSPICE: bool):
        ...
        aSTEAM = AnalysisSTEAM(file_name_analysis=file_name_analysis)
    ...
    def __get_circuit_type(self, circuit_name: str):
        if circuit_name.startswith("RCBH") or circuit_name.startswith("RCBV"):
            ...
            magnet_number = self.__find_magnets(os.path.abspath(os.path.join(os.getcwd(), "../../../steam_models/circuits/circuit_parameters/RQ_circuit_parameters.csv")), circuit_name)
            ...
            magnet_number = self.__find_magnets(os.path.abspath(os.path.join(os.getcwd(), "../../../steam_models/circuits/circuit_parameters/600A_circuit_parameters.csv")), circuit_name)
