...
def classify_dataset_with_kernels(file_path):
    ...
    X, y, X_names, labels = io.open_dataset(file_path) # 'vergalijo_20_varieties.csv'
    ...
    io.draw_class_average(X, y, X_names, labels=labels, SD=1, shift = (X.max() - X.min()) / n_classes, figsize = (round(n_classes * 1.5) + 3, round(n_classes * .5) + 2 ))
    _ = io.scatter_plot(X, y, labels=labels, figsize = (round(n_classes * .5) + 3, round(n_classes * .3) + 2 ))
