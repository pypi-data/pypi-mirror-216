...
def reference_current_function():
    ...
    return extract_call_stack_info(depth=1)[1]
...
def extract_call_stack_info(*, depth=0):
    ...
    func = None
    if caller in frame_info.frame.f_globals:
        func = frame_info.frame.f_globals[caller]
    else:
        for k, obj in frame_info.frame.f_globals.items():
            if k.startswith('__'):
                continue
