...
def results(run_id, y_true = [], y_predicted = [], results_dict = {}, node_name="Results", problem_type = 'binary classification', hist_bins=20):
    ...
    if len(y_true) != 0 and len(y_predicted) != 0:
      ...
      binary = {'precision': round(prec, 4), 'recall': round(rec, 4), 'specificity': round(spec, 4),
              ...
              'test_auc': roc_auc, 'freq': freq, 'bins': hist_bins, 'fpr': fpr, 'tpr': tpr}
