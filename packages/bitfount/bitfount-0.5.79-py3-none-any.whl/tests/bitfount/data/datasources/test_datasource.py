...
if tuple(map(int, platform.python_version_tuple()[:2])) < (3, 10):
    # Intermine does not support Python 3.10 and above
    from intermine.webservice import Service

    from bitfount.data.datasources.intermine_source import IntermineSource
else:

    class IntermineSource:  # type: ignore[no-redef] # Reason: Used for typing
        """Dummy class for typing of IntermineSource."""
...
@unit_test
@pytest.mark.skipif(
    importlib.util.find_spec("intermine") is None,
    reason="Skipping the test as intermine package is not available.",
)
