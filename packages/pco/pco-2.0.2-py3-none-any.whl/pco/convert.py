...
class Convert:
    def __init__(self, camera_handle, sdk, convert_type, bit_resolution):
        if sys.platform.startswith('win32'):
            self._dll_name = "pco_conv.dll"
        elif sys.platform.startswith('linux'):
            self._dll_name = "libpco_convert.so.1"
        else:
        ...
        try:
            if sys.platform.startswith('win32'):
                self.PCO_Convert = C.windll.LoadLibrary(dll_path + "/" + self._dll_name)
            else:  # if sys.platform.startswith('linux'):
                self.PCO_Convert = C.cdll.LoadLibrary(dll_path + "/" + self._dll_name)
        except OSError:
            logger.error(
                "Error: "
                + '"'
