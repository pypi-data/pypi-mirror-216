...
def match_response_class(name):
    ...
    def register_class(qname2class, cls):
        name = hasattr(cls, "meta") and hasattr(cls.meta, "name") and cls.meta.name or cls.__name__ 
        qname2class[name] = cls

    # TODO make it work with cte, mdfe...

    qname2class = {}

    # taking the root classes and their 1st children is enough
    for _klass_name, klass in nfe_bindings.__dict__.items():
        if isinstance(klass, type) and type(klass) != EnumMeta:
            classes = set()
            cls = klass
            classes.add(cls)
            register_class(qname2class, cls)
            for attr in dir(cls):  # only add 1st level children
