...
def match_response_class(name):
    ...
    def qname_or_name(cls):
        return hasattr(cls, "meta") and hasattr(cls.meta, "name") and cls.meta.name or cls.__name__ 
    ...
    for _klass_name, klass in nfe_bindings.__dict__.items():
        if isinstance(klass, type) and type(klass) != EnumMeta:
            ...
            cls = klass
            classes.add(cls)
            qname2class[qname_or_name(cls)] = cls
            for attr in dir(cls):  # only add 1st level children
                ...
                if (
                    ...
                    continue
                qname2class[qname_or_name(nested)] = nested
                classes.add(nested)
