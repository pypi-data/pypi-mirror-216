...
class Worker:
    ...
    async def publish_msg(
        self, msg: bytes, *, subject, headers: Optional[dict] = None
    ):
        async with self.connect() as connection:
            await publish_msg(
                connection=connection,
    ...
    async def get_value(self, *, key: str, bucket: str) -> NatsKeyValue.Entry:
        async with self.connect() as connection:
    ...
    def delete_value(self, bucket, key):
        with self.connect() as connection:
            return delete_value(connection=connection, bucket=bucket, key=key)

    def purge_stream(self, subject):
        with self.connect() as connection:
            return purge_stream(connection=connection, subject=subject)
...
async def get_value(
    *, connection: NatsClient, key: str, bucket: str
