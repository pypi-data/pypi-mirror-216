...
class FitWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, dataset: TauTDataset | TauHDataset, usel: object,
        ...
        self.x_grid = np.linspace(
            np.min(x_vals - self.logshift),
            np.max(x_vals - self.logshift),
            1000
        )
        ...
        gui.convert_log_ticks_to_lin(ax, np.log10(x_vals), shift=self.logshift)
...
def _plot_fitted_times(dataset: TauTDataset | TauHDataset,
    ...
    for logmodel in logmodels:
        ...
        ax.plot(
            1. / np.array(x_vars_grid),
            np.log(1. / np.array(model_rates)),
            lw=1.5,
            label=logmodel.NAME,
            ls='--'
        )
