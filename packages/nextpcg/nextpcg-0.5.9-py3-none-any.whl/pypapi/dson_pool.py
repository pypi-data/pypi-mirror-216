...
class DsonPool(metaclass=Singleton):
    ...
    class Plugin:
        ...
        #
        # def dummy(self):
        #     return DsonPool.Plugin(self.name)
    ...
    async def __worker_loop(self, i):
        ...
        while True:
            ...
            try:
                ...
                await plugin.lock.acquire() # plugin lock
                running_coro = asyncio.wait_for(task_to_proceed(plugin.proc, *args), self._max_task_time, loop=self._loop)
                await running_coro
                ...
                if got_exception:
                    ...
                    try:
                        ...
                        # get_plugin may raise error, so plugin is unbound. We don't need to solve this, plugin deletion should be
                        # done by get_plugin() in this case
                        pass
                else:
                    try:
                        plugin.lock.release()
    ...
    async def get_plugin(self, dson_meta_info: DsonMetaInfo):
        ...
        if plugin_name not in self._plugins:
            ...
            bash_path = os.path.join(plugin_path, dson_plugin_execute_folder, self.dson_script_name).replace('\\', '/')
            if os.name == 'posix':
            ...
            lock = asyncio.Lock()
            plugin = DsonPool.Plugin(plugin_name, proc, lock, self.plugin_start_times)
