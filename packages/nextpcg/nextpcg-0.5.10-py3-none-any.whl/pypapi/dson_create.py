...
def generate_dson_files(base_path, plugin_name):
    index = 0
    dson_path = os.path.join(base_path, dson_plugin_file_folder)
...
def generate_dson_init_script(base_path, conda_env):
    if os.name != 'posix':
        # windows
        ...
        script_string = f"""#! /usr/bin/bash
                        ...
                        python dson_main.py
                        """
    nextpcg_path = os.path.join(base_path, dson_plugin_execute_folder)
