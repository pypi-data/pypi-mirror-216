...
def get_supported_plugins():
    ...
    file_path = os.path.join(current_dir, 'pypi_client', 'openplugin_client', 'plugins.json')
...
def openplugin_completion(early_access_token: str, plugin_name: str = None, fail_silently: bool = False, **chatgpt_args):
    ...
    if plugin_name and plugin_name not in supported_plugins:
        raise ValueError(f"Unsupported plugin '{plugin_name}'. For full list of supported plugins, visit, https://github.com/CakeCrusher/openplugin-clients/blob/master/PLUGINS.md .")
