...
def authenticate_user(pw_hash_file: Path) -> bytes:
    if pw_hash_file.exists():
        pw_hash, salt = get_local_pw_hash(pw_hash_file)
    else:
        pw_hash, salt = application_password_prompt_new(pw_hash_file)

    provided_password: str = application_password_prompt(pw_hash)
    application_key: bytes = key_from_password(provided_password, salt=salt)
