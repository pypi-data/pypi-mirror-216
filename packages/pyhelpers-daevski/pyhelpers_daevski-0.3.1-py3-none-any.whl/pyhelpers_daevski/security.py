...
def key_from_password(password_provided: str, salt: bytes) -> bytes:
    password = password_provided.encode()  # Convert to type bytes
    # TODO: Diff salt for each site password; generate_salt method needed.
    _salt = hashlib.sha256(salt).hexdigest().encode('ascii')
    kdf = PBKDF2HMAC(
...
def get_local_pw_hash(file_path: Path) -> tuple[str, bytes]:
    salt_file = Path(f"{file_path}.salt")
...
def create_hash(provided_password: str, pw_hash_file: Path) -> tuple[str, bytes]:
    salt_file = Path(pw_hash_file) / ".salt"
    if not pw_hash_file.parent.exists():
        pw_hash_file.parent.mkdir(parents=True)

    hash: str = hash_password(provided_password)
    salt: bytes = os.urandom(SALT_LENGTH)

    with pw_hash_file.open('wb') as f:
        f.write(hash)
...
def application_password_prompt_new(pw_hash_file: Path) -> tuple[str, bytes]:
    _prompt_text = (
    ...
    pwpash, salt = create_hash(provided_password, pw_hash_file)
    return pwpash, salt
