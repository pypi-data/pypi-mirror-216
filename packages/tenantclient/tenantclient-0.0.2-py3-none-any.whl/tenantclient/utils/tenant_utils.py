...
from tenantclient.constants import TENANT_ID
...
def get_current_tenant():
    """
    Utils to get the tenant that has been set in the current thread using `set_current_tenant`.
    ...
    """
    return ThreadContainer.get_value(TENANT_ID)  # getattr(_thread_locals, 'tenant', None)
...
def set_object_tenant(instance, value):
    if hasattr(instance, 'tenant_value'):
...
def unset_current_tenant():
    ThreadContainer.set_value(TENANT_ID, None)
...
def get_tenant_timezone():
    # TODO read from tenant settings
    return 'Asia/Kolkata'
