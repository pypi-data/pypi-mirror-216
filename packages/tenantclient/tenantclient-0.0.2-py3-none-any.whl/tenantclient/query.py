...
from .utils.tenant_utils import (get_current_tenant, get_tenant_column,
                                 get_tenant_filters, is_distributed_model, set_object_tenant)
...
def wrap_insert_batch(base_insert_batch):
    def insert_batch(obj, fields, objs, raw=False):
        current_tenant_id = get_current_tenant()
        for item in objs:
            set_object_tenant(item, current_tenant_id)
        obj.fields = fields
        obj.objs = objs
        obj.raw = raw

    insert_batch._sign = "insert_batch django-multitenant"
    return insert_batch
