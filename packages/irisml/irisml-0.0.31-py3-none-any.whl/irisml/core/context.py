...
class Context:
    ...
    def add_execution_log(self, name, task_name, task_version, task_hash, config_dict, inputs_dict, cache_hit):
        self._execution_logger.log(name, task_name, task_version, task_hash, self._resolve_primitive(config_dict), self._resolve_primitive(inputs_dict), cache_hit)

    def clone(self):
        return copy.deepcopy(self)

    def _resolve_primitive(self, value):
        """Recursively replace Variables with actual value. Only replace primitive types.

           If the value is not a primitive type, return the variable name as a string.
        """
        if isinstance(value, dict):
            return {k: self._resolve_primitive(v) for k, v in value.items()}
        elif dataclasses.is_dataclass(value):
            return {field.name: self._resolve_primitive(getattr(value, field.name)) for field in dataclasses.fields(value)}
        elif isinstance(value, list):
            return [self._resolve_primitive(v) for v in value]
        elif isinstance(value, Variable):
            try:
