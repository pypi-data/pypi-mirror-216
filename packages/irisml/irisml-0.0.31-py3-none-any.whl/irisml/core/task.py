...
class Task:
    ...
    def execute(self, context, dry_run=False):
        ...
        task_hash = None
        if context.is_cache_enabled:
            task_hash = HashGenerator.calculate_hash([config, inputs], context)

        if task_hash and self._task_class.CACHE_ENABLED:
            ...
            if cached_outputs:
                ...
                context.add_execution_log(self.name, self.task_name, self._task_class.VERSION, task_hash, config, inputs, cache_hit=True)
                return cached_outputs

        context.add_execution_log(self.name, self.task_name, self._task_class.VERSION, task_hash, config, inputs, cache_hit=False)
