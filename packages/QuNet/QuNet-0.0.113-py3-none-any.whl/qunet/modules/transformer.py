...
class  Transformer(nn.Module):
    ...
    def plot(self, w=12, h=3, eps=1e-8, bar_width = 0.25, info=False):
        ...
        gamma_d = [] 
        for i,(f,a,m) in enumerate(zip(fft_gamma_d, att_gamma_d, mlp_gamma_d)):
            gamma_d.append([i,f]); gamma_d.append([i+bar_width,a]); gamma_d.append([i+2*bar_width,m]); 
        gamma_d = np.array(gamma_d).transpose()
        
        ax2.plot(gamma_d[0], gamma_d[1], "-b.")
        ...
        gamma_g = [] 
        for i,(f,a,m) in enumerate(zip(fft_gamma_g, att_gamma_g, mlp_gamma_g)):
            gamma_g.append([i,f]); gamma_g.append([i+bar_width,a]); gamma_g.append([i+2*bar_width,m]); 
        gamma_g = np.array(gamma_g).transpose()
        
        ax3.plot(gamma_g[0], gamma_g[1], ":b.")
        ...
        if info:
            return {
                'fft_dv': fft_dv, 'fft_gamma_d': fft_gamma_d,
                'att_dv': att_dv, 'att_gamma_d': att_gamma_d,
                'mlp_dv': mlp_dv, 'mlp_gamma_d': mlp_gamma_d,
            }
    ...
    def unit_test():
        B, T, E, H = 1, 10, 128, 16
        m = Transformer(E=E, H=H, n_blocks=10, is_fft=1, is_att=1)
