...
class Attention(nn.Module):
    ...
    def set_drop(self, value):
        self.att_dropout.p = value
        self.res_dropout.p = value
...
class  TransformerBlock(nn.Module):
    ...
    def set_drop(self, fft, att, mlp):
        if self.cfg.is_fft:
            self.fft.module.set_drop( fft )

        if self.cfg.is_att:
            self.att.module.set_drop( att )

        if self.cfg.is_mlp:
            self.mlp.module.set_drop( mlp )
