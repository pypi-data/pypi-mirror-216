...
class ResBlockCNN(nn.Module):
        ...
        if self.align is  None:
            ...
            if self.debug:       
                ...
                if self.training and self.std > 0:
                    x = x + dx * (1+torch.randn(1, device=x.device)*self.std)
                else:
                    x = x + dx 

            else:
                if self.training and self.std > 0:
                    x =  self.align(x)  + self.block(x) * (1+torch.randn(1, device=x.device)*self.std)
                else:
    ...
    def add_hook(self):
        if self.__hook is None:
            self.__hook = self.block.register_full_backward_hook(self.backward_hook)
        ...
        for block in self.layers:
            if type(block) is ResBlockCNN:
                ...
                for layer in block.block:
                    if type(layer) == nn.Conv2d:
                        ww.append(torch.sqrt(torch.square(layer.weight.data).mean()).cpu().item())
                weights.append(ww)
                ma = max(ma, len(ww))
...
class ResBlockCNN_old(nn.Module):
    def __init__(self, *args, **kvargs):
        """
        ...
        """
        super().__init__()
        self.cfg = ResBlockCNN_old.default()
        self.cfg.set(*args, **kvargs)
        self.create()
...
class ResCNN_old(nn.Module):
    ...
    def create(self):
        ...
        self.layers = []
        for i in range(len(channels)-1):
            self.layers +=  [ ResBlockCNN_old(in_channels=channels[i],
                                          out_channels=channels[i+1],
                                          kernel=cfg.kernel[i],
                                          skip=cfg.skip[i], n_layers=cfg.layer[i],
                                          stride=cfg.stride[i],
