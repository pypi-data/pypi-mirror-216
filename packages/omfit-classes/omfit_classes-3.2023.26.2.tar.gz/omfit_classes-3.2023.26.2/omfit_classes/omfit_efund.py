...
class OMFITmhdin(OMFITnamelist):
    ...
    def from_omas(self, ods, passive_map='F'):
        ...
        # ITER
        coil_map['CS'] = 'F'  # Central solenoid
        # coil_map['CS'] = 'E' # probably a better choice once operational values are known
        ...
        if 'magnetics' in ods:
            if 'b_field_pol_probe' in ods['magnetics']:
                self['MACHINEIN']['magpr2'] = nmp2 = len(list(filter(None, ods['magnetics.b_field_pol_probe.:.length'])))
...
def green_to_omas(
    ...
    :param magpr2: number of magnetic probes (will be overwritten if available in directory)
    ...
    rmp2fc = f.read_reals(f'{endian}f8').reshape([nfsum, magpr2]).T
    ...
    try:
        ...
        rmp2ec = f.read_reals(f'{endian}f8').reshape([nesum, magpr2]).T
        gridec = f.read_reals(f'{endian}f8').reshape([nesum, mw * mh]).T
        ods['em_coupling.mutual_loops_active'] = np.append(rsilfc, rsilec, axis=1)
        ods['em_coupling.field_probes_active'] = np.append(rmp2fc, rmp2ec, axis=1)
        ods['em_coupling.mutual_grid_active'] = np.append(ggridfc, gridec, axis=1)
        no_ecoil = False
    except Exception:
    ...
    try:
        ...
        ods['em_coupling.field_probes_passive'] = f.read_reals(f'{endian}f8').reshape([nvsum, magpr2]).T
    ...
    try:
        ...
        if not no_ecoil:
            ods['em_coupling.mutual_passive_active'] = np.append(gfcvs, gecvs, axis=1)
        else:
    ...
    ods['em_coupling.field_probes_grid'] = f.read_reals(f'{endian}f8').reshape([mw * mh, magpr2]).T
