...
from azureml.mlflow._internal.utils import ServiceContext
...
def artifact_uri_decomp(artifact_uri):
    ...
    if not artifact_info_dict:
        ...
        artifact_info_dict[_ARTIFACT_PATH] = artifact_path if not artifact_path.startswith("/") else artifact_path[1:]
...
def get_service_context_from_tracking_url_default_auth(parsed_url):
    ...
    auth = DefaultAzureCredential(exclude_interactive_browser_credential=False,
                                  exclude_managed_identity_credential=True,
                                  exclude_environment_credential=True)

    service_context = ServiceContext(
        subscription_id=subscription_id, resource_group_name=resource_group_name,
        workspace_name=workspace_name, auth=auth, host_url="https://" + parsed_url.netloc)
...
def get_service_context_from_tracking_url_mlflow_env_vars(parsed_url):
    ...
    service_context = ServiceContext(
        subscription_id=subscription_id, resource_group_name=resource_group_name,
        workspace_name=workspace_name, auth=auth, host_url=host_url
    )
    # TODO Create host url from tracking URL select scheme based on dev mode or no
...
def get_artifact_repository_client(artifact_uri):
    ...
    if _EXP_NAME in parsed_artifacts_path and _RUN_ID in parsed_artifacts_path:
        ...
        artifacts_client = RunArtifactsClient(service_context, experiment_name, run_id, path)
    else:
        origin = parsed_artifacts_path[_ORIGIN]
        container = parsed_artifacts_path[_CONTAINER]
        path = parsed_artifacts_path[_ARTIFACT_PATH]
        artifacts_client = LocalArtifactClient(service_context, origin, container, path)
