...
class ModflowHob(Package):
    ...
    def _set_dimensions(self):
        ...
        for idx, obs in enumerate(self.obs_data):
            if not isinstance(obs, HeadObservation):
                msg += (
                    f"ModflowHob: obs_data entry {idx} "
    ...
    def write_file(self):
        ...
        # write dataset 1
        f.write(f"{self.nh:10d}")
        f.write(f"{self.mobs:10d}")
        f.write(f"{self.maxm:10d}")
        f.write(f"{self.iuhobsv:10d}")
        f.write(f"{self.hobdry:10.4g}")
        if self.no_print or "NOPRINT" in self.options:
            f.write("{: >10}".format("NOPRINT"))
        f.write("\n")

        # write dataset 2
        f.write(f"{self.tomulth:10.4g}\n")
        ...
        for idx, obs in enumerate(self.obs_data):
            ...
            if layer >= 0:
                layer += 1
            line += f"{layer:10d} "
            line += f"{obs.row + 1:10d} "
            line += f"{obs.column + 1:10d} "
            irefsp = obs.irefsp
            if irefsp >= 0:
                irefsp += 1
            line += f"{irefsp:10d} "
            if obs.nobs == 1:
                ...
                hobs = 0.0
            line += f"{toffset:20} "
            line += f"{obs.roff:10.4f} "
            line += f"{obs.coff:10.4f} "
            line += f"{hobs:10.4f} "
            line += f"  # DATASET 3 - Observation {idx + 1}"
            f.write(f"{line}\n")
            ...
            if len(obs.mlay.keys()) > 1:
                ...
                for key, value in iter(obs.mlay.items()):
                    line += f"{key + 1:5d}{value:10.4f}"
                line += f"  # DATASET 4 - Observation {idx + 1}"
                f.write(f"{line}\n")
            ...
            if irefsp < 0:
                line = f"{obs.itt:10d}"
                line += 103 * " "
                line += f"  # DATASET 5 - Observation {idx + 1}"
                f.write(f"{line}\n")
            ...
            if obs.nobs > 1:
                for jdx, t in enumerate(obs.time_series_data):
                    ...
                    if isinstance(obsname, bytes):
                        obsname = obsname.decode("utf-8")
                    line = f"{obsname:12s}   "
                    line += f"{t['irefsp'] + 1:10d} "
                    line += f"{t['toffset']:20} "
                    line += f"{t['hobs']:10.4f} "
                    line += 55 * " "
                    line += f"  # DATASET 6 - Observation {idx + 1}.{jdx + 1}"
                    f.write(f"{line}\n")
