        ...
        for key, analysers in self._analysers.items():
            ...
            if total_tx_bytes:
                total_bytes_relative_loss = \
                    100 * total_bytes_loss / total_tx_bytes_without_vlan
...
def _summarize_analysers(
...
) -> Tuple[bool, int, int, int, int, int, int, Timestamp, Timestamp,
    ...
    :rtype: Tuple[bool, int, int, int, int, int, int, Timestamp, Timestamp,
       Optional[Tuple[ float, float, float, float]]]
    """
    ...
    if layer2_speed is None:
        ...
        pass
    elif layer2_speed == Layer2Speed.frame:
