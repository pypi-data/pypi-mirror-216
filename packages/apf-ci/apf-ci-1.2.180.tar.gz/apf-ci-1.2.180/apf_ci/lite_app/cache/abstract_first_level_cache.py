...
class AbstractFirstLevelCache(AbstractCache, metaclass=ABCMeta):
    @abstractmethod
    ...
    def get_js_package_cache(self, npm_dto, cache_md5, build_path):
        ...
        zip_file_path = os.path.join(module_path, "1_" + zip_file_name)
        dist_path = ""
        ...
        if is_file_invalid(nd_dependencies_file_path):
            error_message = '"ndDependencies.version 文件不存在，path: %s' % nd_dependencies_file_path
            logger.error(LoggerErrorEnum.JENKINS_BUILD_ERROR_001002, error_message)
            sys.exit(1)
            # 压缩
        files_list = []
        ...
        # 保存缓存到本地
        dest_path = os.path.join(upload_cs_path, zip_file_name)
        self.save_local_cache(zip_file_path, dest_path)
    ...
    def save_local_cache(self, src_file_path, dest_path):
        dest_file_path = "/data/jenkins/.apf" + dest_path
        if platform.system() == "Darwin":
            dest_file_path = "/usr/local/sdp/jenkins/.apf" + dest_path
        copy_file(src_file_path, dest_file_path)
        logger.debug(" 保存本地缓存: 拷贝 %s 到 %s" % (src_file_path, dest_file_path))

    def get_cache_md5(self, cache_factor):
        return self.get_first_level_cache_md5(cache_factor)

    def get_cache(self, cache_md5, param_map):
        cache_bo = None

        lite_app_host = param_map["lite_app_server"]
        cache_url = self.get_cache_url(lite_app_host, "1", cache_md5)
        if cache_url != "":
            cache_bo = CacheBO()
            cache_bo.zip_url = cache_url
            cache_bo.first_level_cache_md5 = cache_md5

            logger.debug(" 取到一级缓存: %s" % cache_bo.tostring_format())

        return cache_bo

    def save_cache(self, npm_dto, cache_md5):
        js_package_cache = self.save_first_level_cache(npm_dto, cache_md5)
        ...
        if cache_url != "":
            ...
            post_for_array(lite_app_url, request_body, False)
