...
window.vm = new Vue({
  ...
  data: {
    ...
    imagePool: null,
    getting: false,
    current_device: localStorage.current_device || "",
    alldevice_list : [],
    starting: false,
    devicePort: 0,
    connecting: false,
  ...
  watch: {
    ...
    current_device: function (newval) {
      if(this.current_device.indexOf('iPhone')!==-1 || this.current_device.indexOf('iOS')!==-1){
        this.platform = 'ios'
      }
      else if(this.current_device.toLowerCase().indexOf("windows")!==-1){
        this.platform = "windows"
      }
      else{
        this.platform = 'android'
      }
      this.serial = this.current_device.split('|')[1]
      localStorage.setItem('platform', this.platform);
      localStorage.setItem('current_device', newval);
  ...
  mounted: function () {
    ...
    let params = GetRequestParam(window.location.search)
    this.platform = params['platform'] || localStorage.platform || ""
    this.serial = params['serial'] || localStorage.serial || ""
    if (this.platform==="iOS"){
      this.serial = params['wdaurl'] || localStorage.serial || ""
    }
  ...
  methods: {
    startWda: function (){
      this.starting = true
      $.ajax({
        url: LOCAL_URL + "api/v1/startWDA",
        method: 'POST',
        data: {
          serial: this.current_device.split('|')[1]
        },
      })
        .done((ret) => {
          this.devicePort = ret.devicePort;
          this.serial = "http://localhost:"+this.devicePort
          this.starting = true
        })
        .fail((ret) => {
          this.showError(ret.description);
          this.starting = false;
        })
        .always(() => {
        })
    ...
    getAllDeviceList: function (){
      ...
      .done((ret) => {
          this.alldevice_list = ret.data;
          for(let index=0;index<this.alldevice_list.length;index++){
            let device = this.alldevice_list[index]
            if (device.indexOf(this.serial) !== -1 || device===this.current_device) {
    ...
    checkVersion: function () {
      $.ajax({
        ...
        .fail((ret) => {
          this.showError("<p>Local server not started, start with</p><pre>$ python -m yyperf</pre>");
        })
        .always(() => {
          this.loading = false;
        })
