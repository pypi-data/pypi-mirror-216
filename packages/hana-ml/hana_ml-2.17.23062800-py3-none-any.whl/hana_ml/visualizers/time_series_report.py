...
class TimeSeriesExplainer(object):
    ...
    def set_forecasted_result_explainer(self, forecasted_result_explainer: dataframe.DataFrame, reason_code_name='EXOGENOUS'):
        ...
        self.forecasted_result_explainer_key_data = list(self.forecasted_result_explainer_pd_df[self.key].astype(str))
...
class DatasetAnalysis(object):
    ...
    def moving_average_item(self, rolling_window):
        ...
        X.insert(0, self.key)
        Y1 = list(data_.iloc[:, 1].astype(float))
        Y1.insert(0, data_.columns[1])
        Y2 = list(data_.iloc[:, 2].astype(float))
    ...
    def rolling_stddev_item(self, rolling_window):
        ...
        X = list(data_.iloc[:, 0].astype(str))
        X.insert(0, self.key)
        Y = list(data_.iloc[:, 2].astype(float))
    ...
    def seasonal_item(self):
        ...
        for line_to_plot in lines_to_plot:
            ...
            X.insert(0, 'MONTH')
            Y = list(temp_pf.iloc[:, 1].astype(float))
        ...
        if isinstance(cp_object, BCPD):
            ...
            if tcp.shape[0] > 0:
                ...
                if cp_style == "scatter":
                    ...
                    X = list(result[title].astype(str))
                    x_data.extend(X)
                    X.insert(0, title)
                    Y = list(result[self.endog])
                    Y.insert(0, self.endog)
                    datasetIndex = datasetIndex + 1
                    datasets.append({
                        'source': [X, Y]
            ...
            if display_trend is True:
                ...
                X = list(result[self.key].astype(str))
                x_data.extend(X)
                X.insert(0, self.key)
                Y = list(result["TREND"])
                Y.insert(0, 'TREND')
