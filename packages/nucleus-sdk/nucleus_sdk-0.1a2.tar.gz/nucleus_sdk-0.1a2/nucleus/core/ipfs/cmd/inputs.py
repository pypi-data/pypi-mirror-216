...
class Dir:
    """Dir represent dir params in request based on input path.
    ...
    def __iter__(self):
        directory = [('file', (self.path, str(self.path.absolute()), 'application/x-directory'))]
        files = [('file', (str(child), child.read_bytes())) for child in self.path.iterdir()]
        yield 'files', [*directory, *files]
