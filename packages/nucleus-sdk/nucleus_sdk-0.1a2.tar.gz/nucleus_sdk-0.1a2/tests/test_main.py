...
def test_main():
    ...
    with logger.console.status('Processing'):
        ...
        output_path = Path(f'{output_directory}/index.m3u8')
        if not output_directory.exists():
            output_directory.mkdir()

        # output HLS files to the output path
    ...
    with logger.console.status('Expose'):
        ...
        sep001.add_metadata(Structural(cid=stored_file_object.hash, path='index.m3u8'))
