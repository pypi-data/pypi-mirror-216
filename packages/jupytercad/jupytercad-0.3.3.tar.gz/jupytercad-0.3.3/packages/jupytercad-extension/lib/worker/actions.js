 ...
 * }}
 */
export function shapeToThree(shapeData) {
    const oc = getOcc();
    ...
    shapeData.forEach(data => {
        ...
        new oc.BRepMesh_IncrementalMesh_2(shape, maxDeviation, false, maxDeviation * 5, true);
        ...
        while (expl.More()) {
            ...
            if (myT.get().HasUVNodes()) {
                ...
                const UVNodes = myT.get().UVNodes(), UVNodesLength = UVNodes.Length();
                ...
                for (let i = 0; i < UVNodesLength; i++) {
                    let x = thisFace.uv_coord[i * 2 + 0], y = thisFace.uv_coord[i * 2 + 1];
                    x = (x - UMin) / (UMax - UMin);
                    y = (y - VMin) / (VMax - VMin);
...
function buildModel(model) {
    ...
    objects.forEach(object => {
        ...
        if (ShapesFactory[shape]) {
            const shapeData = ShapesFactory[shape](parameters, model);
            if (shapeData) {
