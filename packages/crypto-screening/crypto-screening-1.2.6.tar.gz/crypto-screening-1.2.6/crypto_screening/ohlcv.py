...
class OHLCVScreener(BaseScreener):
    ...
    async def async_get_market(self) -> Dict[str, Number]:
        ...
        data[self.symbol][VOLUME.lower()] = data[self.symbol]["quoteVolume"]
        data[self.symbol][ASKS.lower()] = data[self.symbol]["ask"]
        data[self.symbol][BIDS.lower()] = data[self.symbol]["bid"]

        data = {
            key: data[self.symbol][key.lower()] for key in
            self.COLUMNS
        }

        if any(
            (np.isnan(data[column]) or (data[column] is None))
            ...
            ohlcv = {
                column: value for column, value in zip(OHLCV_COLUMNS, ohlcv)
