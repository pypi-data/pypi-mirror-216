...
class DeBERTaClassifier(nn.Module):
    def __init__(self, hyperparameters, path=None, num_labels=2):
        ...
        if path:
            self.deberta = AutoModelForSequenceClassification.from_pretrained(path, num_labels=num_labels)
        else:
            self.deberta = AutoModelForSequenceClassification.from_pretrained('microsoft/deberta-base', num_labels=num_labels)
...
class BertClassifier(nn.Module):
    def __init__(self, hyperparameters, path=None, num_labels=2):
        ...
        self.batch_size = hyperparameters["batch-size"]
        if path:
            self.bert = AutoModelForSequenceClassification.from_pretrained(path, num_labels=num_labels)
        else:
            self.bert = AutoModelForSequenceClassification.from_pretrained('bert-base-cased', num_labels=num_labels)
        self.labels = {"CON": 0, "PRO":1}
