...
class Dataset(torch.utils.data.Dataset):
    def __init__(self,dataset, path=None, model='bert-base-cased'):
        topics, texts, stances = dataset["topic"].values, dataset["text"].values, dataset["stance"].values
        if path:
...
def run_ibm_baseline(params, experiment, validate=True):
    ...
    topics, texts, labels = topics, texts, stances = test_split["topic"].values, test_split["text"].values, test_split["stance"].values
    list_of_instances = []
