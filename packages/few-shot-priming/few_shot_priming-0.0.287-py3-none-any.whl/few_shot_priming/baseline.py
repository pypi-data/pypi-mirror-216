...
def baseline(config, experiment, offline=True, validate=True, majority=False):
    ...
    pro_f1 = f1_score(labels, predictions, average='macro', pos_label=1)
    con_f1 = f1_score(labels, predictions, average='macro', pos_label=0)
    if experiment == "vast":
        neutral_f1 = f1_score(labels, predictions, average="macro", pos_label=2)
    else:
        neutral_f1 = None
    macro_f1 = f1_score(labels, predictions, average="macro")
    accuracy = accuracy_score(labels,predictions)

    return accuracy, pro_f1, con_f1, neutral_f1, macro_f1
...
def run_ibm_baseline(params, experiment, validate=True):
    ...
    topics, texts, labels = zip(*test_split)
    list_of_instances = []
...
if __name__ == "__main__":
    ...
    if args.vast:
        ...
        experiment = "ibmsc"
    if args.random:
        accuracy, pro_f1, con_f1, macro_f1  = baseline(config, experiment= experiment, offline=args.offline, validate=args.validate)
        print(f"random baseline: accuracy {accuracy}, macro-f1 {macro_f1}, pro f1 {pro_f1}, con f1 {con_f1}")
    elif args.majority:
        accuracy, pro_f1, con_f1, macro_f1  = baseline(config, experiment= experiment, offline=args.offline, validate=args.validate, majority=True)
        print(f"majority baseline: accuracy {accuracy}, macro-f1 {macro_f1}, pro f1 {pro_f1}, con f1 {con_f1}")
    elif args.ibm_api:
        accuracy, pro_f1, con_f1, macro_f1  = run_ibm_baseline(config, experiment= experiment, validate=args.validate)
