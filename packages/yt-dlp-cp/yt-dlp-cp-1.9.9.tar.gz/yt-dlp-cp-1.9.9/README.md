...
```python
from yt_dlp_cp import YoutubeDL
...
Most likely, you'll want to use various options. For a list of options available, have a look at [`yt_dlp/YoutubeDL.py`](yt_dlp_cp/YoutubeDL.py#L184).
...
# ℹ️ See help(yt_dlp_cp.YoutubeDL) for a list of available options and public functions
ydl_opts = {}
with yt_dlp_cp.YoutubeDL(ydl_opts) as ydl:
...
with yt_dlp_cp.YoutubeDL() as ydl:
...
ydl_opts = {
  ...
  # ℹ️ See help(yt_dlp_cp.postprocessor) for a list of available Postprocessors and their arguments
...
# ℹ️ See "progress_hooks" in help(yt_dlp_cp.YoutubeDL)
def my_hook(d):
...
# ℹ️ See help(yt_dlp_cp.postprocessor.PostProcessor)
class MyCustomPP(yt_dlp_cp.postprocessor.PostProcessor):
  def run(self, info):
...
with yt_dlp_cp.YoutubeDL() as ydl:
  # ℹ️ "when" can take any value in yt_dlp_cp.utils.POSTPROCESS_WHEN
