...
def main():
    lazy_extractors_filename = get_filename_args(default_outfile='yt_dlp_cp/extractor/lazy_extractors.py')
    if os.path.exists(lazy_extractors_filename):
        os.remove(lazy_extractors_filename)

    _ALL_CLASSES = get_all_ies()  # Must be before import

    import yt_dlp_cp.plugins
    from yt_dlp_cp.extractor.common import InfoExtractor, SearchInfoExtractor

    # Filter out plugins
    _ALL_CLASSES = [cls for cls in _ALL_CLASSES if not cls.__module__.startswith(f'{yt_dlp_cp.plugins.PACKAGE_NAME}.')]
...
def get_all_ies():
    ...
    try:
        from yt_dlp_cp.extractor.extractors import _ALL_CLASSES
    finally:
