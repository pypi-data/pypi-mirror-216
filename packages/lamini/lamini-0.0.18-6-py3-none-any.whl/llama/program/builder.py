...
from llama.program.operations.train_operation import TrainOperation
...
class Builder:
    ...
    def __init__(self, id, model_name=None, config={}):
        self.id = id
        self.program = Program(self, id)
    ...
    def __call__(self, input, output_type, *args, **kwargs):
        # Reset program
        self.program = Program(self, self.id)
        self.current_function = self.program.main
    ...
    def get_job_results(self, job_id, output_type=None):
    ...
    def submit_training_job(self, **kwargs):
        ...
        results = gen_submit_training_job(
            value, self.id, self.model_name, kwargs.get("prompt_key", None)
        )
        temp_job_id = results["job_id"]

        # check inference job status
        status = self.check_job_status(temp_job_id)
        while status["status"] not in ("RUNNING", "DONE", "CREATED"):
            if kwargs.get("verbose", False):
                print(f"preparing data for training...")
            time.sleep(10)
            status = self.check_job_status(temp_job_id)

        results = self.get_job_results(temp_job_id)
        if kwargs.get("verbose", False):
            print(f'finetuning model: {results["finetune_model"]}')
        real_job_id = results["train_job_id"]
        # wait until data part is done
        results = self.get_training_job_status(real_job_id)
        return results
