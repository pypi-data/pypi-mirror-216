...
class RequestValidator(object):
    ...
    def is_valid(self):
        ...
        if 'n_matches' in self.validators:

            if self.n_matched_requests != self.validators['n_matches']:
                self.errors.append(
                    {'details': f'Expected {self.validators["n_matches"]} requests, got {self.n_matched_requests}'})

        elif 'n_matches_lt' in self.validators or 'n_matches_gt' in self.validators:

            if 'n_matches_lt' in 'validators':
                if not self.n_matched_requests < self.validators['n_matches_lt']:
                    self.errors.append(
                        {'details': f'Expected less than {self.validators["n_matches_lt"]} requests, got {self.n_matched_requests}'})
            if 'n_matches_gt' in 'validators':
                if not self.n_matched_requests > self.validators['n_matches_gt']:
                    self.errors.append(
                        {'details': f'Expected more than {self.validators["n_matches_lt"]} requests, got {self.n_matched_requests}'})
