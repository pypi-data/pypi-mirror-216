...
class Model:
    ...
    :properties
        ...
        usecsv(bool) :
            When reading a model, it automatically converts data into a csv file so that Input and Output items are quickly converted to DataFrame
            when model is opened, auto convert '.dat' to '.csv'
    ...
    def __ParseParameters(self, desc):
        ...
        list = desc.splitlines()

        for textline in list[3:]:
            idx = textline.find(' = ')
            if idx < 0:
                continue
    ...
    def __getInputItemByCsvfile(self, key):
        csvpath = os.path.join(self.__engine.ModelDirectory, 'csv', self.name, key + '.csv')
    ...
    def __getOutputItemByCsvfile(self, key, exp_name='Experiment 1', rst_name='Result 0'):
        csvpath = os.path.join(self.__engine.ModelDirectory, 'csv', self.name, exp_name, rst_name, key + '.csv')
        df = pd.read_csv(csvpath)
        return df
