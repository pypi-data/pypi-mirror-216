...
class DataPreProcessing:
    ...
    def get_urls(sentence):
        return " ".join(
            [word for word in sentence.split() if word.lower().startswith("http")]
        )

    @staticmethod
    def remove_urls(sentence):
        return " ".join(
            [word for word in sentence.split() if not word.lower().startswith("http")]
        )
    ...
    def remove_punct(sentence):
        text = " ".join(
            [char for char in sentence.split() if char not in string.punctuation]
        )
    ...
    def fit_transform(self, X):
        if not isinstance(X, pd.DataFrame):
            X = pd.DataFrame(X)
        result = X.loc[:, self.tweets_column].apply(self.remove_urls).apply(
            self.remove_hashtags
        ).apply(self.remove_mentions).apply(self.remove_punct).str.lower().apply(
            self.tokenization
        ).apply(
...
class Modeling:
    ...
    def fit(self, X, y):
        ...
        clf = linear_model.SGDClassifier(
            n_jobs=-1, max_iter=1000, tol=1e-4, n_iter=None
        )
        model_calibrated = CalibratedClassifierCV(
            base_estimator=clf, cv=3, method="sigmoid"
        )

        self.model = make_pipeline(
            TfidfVectorizer(min_df=0.0, max_df=1.0, use_idf=True, max_features=20000),
            model_calibrated,
        )

        self.model.fit(
            X_train.loc[:, self.tweets_column].str.join(" "),
    ...
    def score(self, y_true, y_pred):
        return classification_report(
            self.le.transform(y_true), y_pred, target_names=self.le.classes_
        )
