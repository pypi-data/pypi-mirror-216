...
class Session:
    ...
    :param filelocation: Default file upload location for session.
    ...
    def __init__(self, server_url: str=None,
        ...
        self.filelocation = filelocation or ''
    ...
    def _url_for_resource(self, resource_type: str,
        ...
        if resource_type is not None:
            resource_type_path:str = resource_type.replace('--', '/')
        else:
            resource_type_path:str = resource_type
        url = f'{self.url_prefix}/{resource_type_path}'
    ...
    async def http_request_async(
        ...
        headers = headers or {'Content-Type':'application/vnd.api+json'}
        headers.update(kwargs.pop('headers', {}))
        payload = send_data if isinstance(send_data, bytes) else json.dumps(send_json)
        async with self._aiohttp_session.request(
                http_method, url, data=payload,
                headers=headers,
                **kwargs) as response:
    ...
    def _commit_sync(self, return_resources: bool = False) -> None:
        self.assert_sync()
    ...
    async def _commit_async(self, return_resources: bool = False) -> None:
        self.assert_async()
        logger.info('Committing dirty resources')
        if return_resources:
            resources = []
    ...
    def commit(self, return_resources: bool = False) -> Optional[Awaitable]:
        """
        ...
        """
        if self.enable_async:
            return self._commit_async(return_resources = return_resources)
        else:
            return self._commit_sync(return_resources = return_resources)
    ...
    def _create_commit_file_sync(
        ...
        filelocation: str = None,
        **more_fields) -> 'ResourceObject':
        filename = fields.get('filename', custom_filename or '')
        filelocation = filelocation or self.filelocation
        res = self.create(type_, fields, **more_fields)
        if not isinstance(send_data, bytes):
            if Path.joinpath(Path(filelocation), Path(filename)).is_file():
                with open(Path.joinpath(Path(filelocation), Path(filename)), mode = 'rb') as f:
                    logger.debug(f'Reading "{Path.joinpath(Path(filelocation), Path(filename))}" file data for upload')
                    send_data = f.read()
            else:
                logger.warning(f'File path "{Path.joinpath(Path(filelocation), Path(filename))}" not Found.', exc_info=False)
        res.commit_file(custom_url, send_data = send_data, filename = custom_filename)
        return res

    async def _create_commit_file_async(
        self,
        type_: str,
        fields: dict,
        custom_url: str,
        ...
        **more_fields) -> 'ResourceObject':
        ...
        if not isinstance(send_data, bytes):
            if Path.joinpath(Path(filelocation), Path(filename)).is_file():
                async with aiofiles.open(Path.joinpath(Path(filelocation), Path(filename)), mode = 'rb') as f:
                    logger.debug(f'Awaiting reading "{Path.joinpath(Path(filelocation), Path(filename))}" file data for upload')
                    send_data = await f.read()
                with open(Path.joinpath(Path(filelocation), Path(filename)), 'rb') as f:
                    send_data = f.read()
            else:
                logger.warning(f'File path "{Path.joinpath(Path(filelocation), Path(filename))}" not Found.', exc_info=False)
        await res.commit_file(custom_url, send_data = send_data, filename = custom_filename)
        return res
    ...
    def create_commit_file(
        ...
        **more_fields
        ...
        """
        Create resource and commit (PUSH) it into server with file upload.
        If session is used with enable_async=True, this needs
        to be awaited.

        :param type_: Use this filename instead of automatically determined one.
        :param fields: Use this filename instead of automatically determined one.
        :param resource_type: Resource type from were the uploaded file will be referenced.
        :param fieldname: Resource reference field name from were the uploaded file 
               will be referenced and that determines allowed file types and upload 
               location on the server.
        :param send_data: Optional bytes data that is passed to server in POST request.
        :param filelocation: Optional local file location path for file upload that is 
               read before upload to server in POST request.
        :param custom_filename: Use this filename on server instead of matching the filename in upload location.
        """
        type_path:str = resource_type.replace('--', '/')
        custom_url = f'{self.url_prefix}/{type_path}/{fieldname}'

        if self.enable_async:
            return self._create_commit_file_async(
                type_, fields, custom_url, send_data, custom_filename, filelocation, **more_fields
            )
        else:
            return self._create_commit_file_sync(
                type_, fields, custom_url, send_data, custom_filename, filelocation, **more_fields
            )
