...
def get_woe_iv(data: pd.DataFrame,
    ...
    Args:
        ...
        dtype:分箱类型,可选参数numerical；categorical，其中numerical：数值型；categorical：分类型；
    ...
    # 按照optbinning分箱
    elif method == 'opt' or trend != 'auto':
        from optbinning import OptimalBinning
        opt = OptimalBinning(name=col, dtype=dtype,
                             solver='cp',  # 算子
                             max_n_bins=max_bins,  # 最大分箱数
                             monotonic_trend=trend  # 设置单调趋势
                             )
        opt.fit_transform(df[col], data[target], metric='indices')
        df[col] = pd.cut(df[col], [-inf] + list(opt.splits) + [inf])
    ...
    result['Total IV'] = result['Total IV'].apply(lambda x: format(x, '.' + str(precision) + 'f'))
