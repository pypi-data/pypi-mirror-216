...
def date_string_change_format(date_string: str,
                              output_format="%Y-%m-%d",
                              input_format="%Y-%m-%d") -> str:
...
def date_string_handler(date_string: str, time_format="%Y-%m-%d") -> datetime:
    ...
    if string_search:
        ...
        return datetime.strptime(date_part, time_format)
    if date_string == "tomorrow":
        return datetime.strptime(
            (datetime.now() + timedelta(days=1)).date().isoformat(), time_format
        )
