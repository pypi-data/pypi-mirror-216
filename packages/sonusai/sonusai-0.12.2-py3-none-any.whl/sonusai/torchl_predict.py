...
options:
    ...
    -b BATCH, --batch BATCH         Batch size. [default: 4]
...
def torch_istft_olsa_hanns(AudioF, n_fft, Rhop=None, window=None, gain=None, trim=True):
    """ Torch implementation of Aaware inv transforms using torch library.
        Hard coded to use onesided, complex input, and default W01hann window, auto norm gain
        AudioF is size [channel, frames, bins]
        Output is [channel, samples]
    """

    if Rhop is None:
        Rhop = int(n_fft // 4)  # hop_length

    if gain is None:
        gain = 2 * Rhop / n_fft  # default gain for tf->itf of 1, accommodates gains in fft and fold below

    n_frames = AudioF.size(-2)

    # Create window, make sure to include gain
    if window is None:
        ncmplx = torch.view_as_real(AudioF[0,0,:].resolve_conj())  # datatype ignoring complex
        window = torch.cat((
            torch.zeros(n_fft//2, dtype=ncmplx.dtype, device=ncmplx.device),
            gain * torch.hann_window(n_fft//2, dtype=ncmplx.dtype, device=ncmplx.device)))
        # rect window
        #window = gain * torch.ones(n_fft, dtype=AudioF.dtype, device=AudioF.device)

    # norm gains = 'ortho' = 1/sqrt(n), 'backward' = 1/n, 'forward' = 1
    AudioF = torch.fft.irfft(AudioF, dim=-1, norm='forward')
    assert AudioF.size(-1) == n_fft
    y_tmp = AudioF * window.view(1, 1, n_fft) # multiply window
    ...
    expected_output_signal_len = n_fft + Rhop * (n_frames - 1)
    y = F.fold(y_tmp, output_size=(expected_output_signal_len, 1), **fold_params)
    y = y.reshape(y.size(0), -1)
    assert y.size(1) == expected_output_signal_len

    if trim:
        y = y[..., n_fft - Rhop:Rhop*n_frames]   # trim pre-samples and post-samples to match aaware tensorflow
...
def main():
    ...
    #logger.info(f'Default device is {device} for model prediction.')
    from lightning.pytorch import Trainer
    trainer = Trainer(default_root_dir=output_dir, accelerator='auto')  # prints avail GPU, TPU, IPU, HPU and selected device
        ...
        if reset:
            ...
            for idx, val in enumerate(p_datagen):
                ...
                if wavdbg:
                    ...
                    tmp = torch.complex(feature[..., :half], feature[..., half:]).permute(2,1,0)  # BxFxCH
                    featwav = itf.execute_all(tmp.squeeze().detach().numpy())
                    write_wav(owav_base + "_feat.wav", featwav, 16000)
                    tmp = torch.complex(val[0][..., :half], val[0][..., half:]).permute(2,1,0)
                    truthwav = itf.execute_all(tmp.squeeze().detach().numpy())
                    write_wav(owav_base + "_truth.wav", truthwav, 16000)
                    tmp = torch.complex(ypred[..., :half], ypred[..., half:]).permute(2, 1, 0)
                    predwav = itf.execute_all(tmp.squeeze().detach().numpy())
                    write_wav(owav_base + ".wav", predwav, 16000)
...
def _pad_and_predict(built_model: Any, feature: Feature) -> (Feature, Predict):
    """
    ...
    noisy_spec = power_compress(torch.view_as_real(feature.permute(1,0,2)))
