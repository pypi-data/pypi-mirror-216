...
def test_create_shadow_dataset_from_storage(listed_bucket, cloud_test_catalog):
    ...
    # ensure the altered table's Table object was reloaded properly
    node_columns = [c.name for c in catalog.data_storage.nodes_table(src_uri).columns]
    assert "test_col" in node_columns
