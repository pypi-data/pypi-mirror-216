...
class Order:
    ...
    def __post_init__(self):
        ...
        ) and self.order_type in (OrderType.LIMIT, OrderType.STOP_LIMIT):
            raise ValueError("Price must be positive decimal if the order_type is LIMIT or STOP_LIMIT")
        if (
                self.stop_price is None
                or self.stop_price < Decimal(0)
        ) and self.order_type in (OrderType.STOP, OrderType.STOP_LIMIT):
            raise ValueError("Stop price must be positive decimal if the order_type is LIMIT or STOP_LIMIT")
