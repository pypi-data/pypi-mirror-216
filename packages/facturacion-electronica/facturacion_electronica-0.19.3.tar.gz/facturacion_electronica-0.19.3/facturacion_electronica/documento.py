...
#                lines['RUTMandante']
    ...
    def impuestos(self, linea):
        ...
        def set_impuesto(val, base=0, cantidad=0, base_otr_mnda=0):
            ...
            imp._base += base
            imp._base_otr_mnda += base_otr_mnda
            imp._compute_tax()
            ...
            if not self.es_boleta():
                ...
                for i in self.impuestos:
                    if i.tax_id.es_adicional:
                        ...
                        ImptoReten['TipoImp'] = i.tax_id.CodImp
                        ImptoReten['TasaImp'] = 1 if i.tax_id.especifico else round(i.tax_id.TasaImp, 2)
                        mnt_imp = util.round0(i.MontoImp)
                        ImptoReten['MontoImp'] = 1 if mnt_imp < 0 else self._aplicar_dsc_rcg(mnt_imp)
                        itemsImptoReten.append({"ImptoReten": ImptoReten})
                if itemsImptoReten:
                    Totales['itemsImptoReten'] = itemsImptoReten
