...
class YbConnector:
    ...
    def loadBySio(self,src,target):
        ...
        buf = StringIO()
        buf.write(src.to_csv(index=None,header=True))
        buf.seek(0)
        
        #Write the data into Yellowbrick.
        conn = self.engine.raw_connection()
        with conn.cursor() as c:
            c.copy_expert(f"COPY {target} FROM STDIN WITH CSV HEADER",buf,size=64*1024) 
        conn.commit()
        conn.close()
    ...
    def loadBySio_pg(self,src,target):
        """ """
        import psycopg2 as pg
        schema,table=target.split(".")[0:2]
        
        #Initialize and load a string buffer.
        buf = StringIO()
        buf.write(src.to_csv(index=None,header=None))
        buf.seek(0)
        
        conn = pg.connect(user=self.user,password=self.pw,database=self.db,host=self.host,options=f'-c search_path={schema}',connect_timeout=300)
        with conn.cursor() as c:
            c.copy_from(buf,table,columns=src.columns,sep=',')
        conn.commit()
        conn.close()
        return
...
def test(timeout_minutes=0.1):
    ...
    print("=== Drop, ctas, and select from a table ===")
    ...
    print("=== Bulkload using ybload and string buffers ===")
