...
def print_sf_term_content(sf_angm, spin_mult, max_angm=13):

    term_composition = \
        {mult: {Term(L=qn, S=(mult - 1) / 2): np.diag(project_SO3_irrep(qn, angm[1].imag, angm[2].imag))
                for qn in range(max_angm)}
         for mult, angm in zip(spin_mult, sf_angm)}

    print("Spin-free section:")
    for mult, composition in term_composition.items():
        print(f"S = {(mult-1)/2}")
        print_terms(composition)


def print_so_term_content(so_spin, so_angm, spin_mults, max_angm=13):

    raise NotImplementedError

    term_composition = \
        {Term(S=(mult - 1) / 2, L=qn): np.diag(project_term_irrep(Term(mult, letter), so_spin, so_angm))
         for mult in spin_mults for qn in range(max_angm)}

    print("Spin-orbit section:")
    print_terms(term_composition)


def print_terms(composition):
    ...
    for term, comp in composition.items():
        ...
        count = np.sum(comp) / (2 * term.qn['L'] + 1)
        if count > 0.1:
    ...
    for state in itertools.count():
        try:
            content = ' + '.join([f"{comp[state]:4.2f}|{term}>"
                                  for term, comp in composition.items()
                                  if comp[state] > 0.1])
            print(f"|State {state+1:3d}> = {content}")
        except IndexError:
            break
    print("=" * 33)
