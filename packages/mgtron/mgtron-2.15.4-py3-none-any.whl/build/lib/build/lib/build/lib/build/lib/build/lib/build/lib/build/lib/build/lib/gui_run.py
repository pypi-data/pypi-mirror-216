...
from gui.helpers import bluetooth_scan_jam
from gui.helpers import custom_save
...
# frosrc.gui.helpers import change_inputs
from gui.helpers import custom_load
from gui.helpers import device_finder
from gui.helpers import device_refresh
from gui.helpers import fill_config
from gui.helpers import kill_channel
from gui.helpers import quick_load
from gui.helpers import quick_save
...
from gui.helpers import wifi_scan_jam
...
def main():
    ...
    with dpg.font_registry():
        ...
        try:  # Stop gap incase the files cannot be found
            ...
            bold_font = dpg.add_font(
                file=f"{ROOT}/src/gui/fonts/MesloLGS NF Bold Italic.ttf",
                size=40)
        ...
        with dpg.window(
            ...
            with dpg.child_window(
                ...
                send_all = dpg.add_button(
                    ...
                    pos=(
                        (
                            dpg.get_item_width(item="big_buttons") - 260) /
                        DIVISOR,
                        (
                ...
                dpg.add_text(
                    ...
                    pos=(
                        (
                            dpg.get_item_width(item="big_buttons") - 200) /
                        DIVISOR,
                        (
            ...
            with dpg.child_window(
                ...
                with dpg.popup(
                    ...
                    [
                        (
                            # Input text, if the card exists, to change the
                            # name of the card
                            dpg.add_input_text(
    ...
    try:  # Stop gap in case the font files cannot be found
        ...
        [
            (
                dpg.bind_item_font(item=f"freq_{i}",
                                   font=bold_font),
                dpg.bind_item_font(item=f"power_{i}",
                                   font=bold_font),
                dpg.bind_item_font(item=f"bandwidth_{i}",
                                   font=bold_font),
    ...
    dpg.create_viewport(
        ...
        x_pos=0,
        y_pos=0,
        small_icon=f"{ROOT}/src.gui/assets/network_wireless.ico",
        large_icon=f"{ROOT}/src.gui/assets/network_wireless.ico",
    )
    dpg.setup_dearpygui()
