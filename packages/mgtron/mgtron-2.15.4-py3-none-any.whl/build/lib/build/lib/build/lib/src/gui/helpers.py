...
from src.gui.db.models import delete_sql_save_data, get_sql_details
from src.gui.db.models import get_sql_save_names
from src.gui.db.models import save_to_database
...
def find_signals_and_frequencies() -> dict[int, float]:
    ...
    if len(matched_sigs_and_freqs) < 8:
        [
            matched_sigs_and_freqs.update({1 + i: 50}) for i in range(8 - len(matched_sigs_and_freqs))
        ]
        loggey.warning(
...
def ble_data_complete() -> dict:
    ...
    # print(f"ble_company > ble_rssi: {True if len(ble[0]) > len(ble_rssi[0]) else False}")

    # print(f"ble: {ble}")

    ble.insert(3, [])

    # print(f"ble len: {len(ble)}")
    # print(f"ble_rssi: {ble_rssi}")

    loggey.info(f"ble_company length: {len(ble[0])}")
    loggey.info(f"ble_rssi length: {len(ble_rssi[0])}")

    try:
        # Iterate through the MAC addresses of the shorter list
        for i, mac in enumerate(ble_rssi[0]):
            mac = mac.split(']')[1]
            # print(f"mac to check for: {mac}\n")
            for mac_2 in ble[0]:
        ...
        loggey.warning(msg=e)

    # [print(f"ble: {j}, len: {len(i)}") for i in ble for j in i]

    # prime for refactoring
    # make all list lengths the same
    for i in range(len(ble)):
        # if the number of MAC addresses is greater than the number of RSSIs

        if len(ble[0]) != len(ble[2]):
            loggey.warn(msg="ble[0] != ble[2]")
            # ble[0].pop()  # remove the last MAC address
            # ble[1].pop()  # remove the last company
            loggey.debug(msg=f"ble prior to insert: {ble}")
            ble[2].append(-174)
            loggey.debug(msg=f"ble after insert: {ble}")
            # print(f"len MACS: {len(ble[0])}, len RSSI: {len(ble[2])}")

    # Make the three element list into a dictionary of key: tuple(str, int)
    ble = dict(zip(ble[0], zip(ble[1], ble[2])))

    # Sort the list by RSSI
    ble = sorted(ble.items(), key=lambda x: x[1][1], reverse=True)

    # [print(f"dict ble: {i}") for i in ble]
