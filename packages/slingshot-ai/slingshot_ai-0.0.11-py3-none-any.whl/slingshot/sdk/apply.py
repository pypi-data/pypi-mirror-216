...
class ApplyService:
    ...
    async def create_app_spec(
        ...
        app_spec_id_response = await self._sdk.create_app(
            ...
            machine_size=app_spec.machine_size,
...
def _get_envs_requiring_gpu(config: schemas.ProjectManifest) -> set[str]:
    ...
    envs_that_require_gpu = {
        app.environment
        for app in apps_runs_deployments
        # TODO: determine if an env requires a GPU so that we don't hardcode this list
        if app.machine_size
        in [
            schemas.MachineSize.GPU,
            schemas.MachineSize.GPU_L4,
            schemas.MachineSize.GPU_A100,
            schemas.MachineSize.GPU_8_A100,
        ]
    }
...
async def remote_project_manifest(sdk: "SlingshotSDK") -> schemas.ProjectManifest:
    ...
    for spec in existing_app_specs:
        ...
        machine_size = schemas.MachineSize(spec.machine_size)
