# bubble-env
bubble区块链部署工具，能够帮你自由的部链、加入、管理bubble区块链


## 安装方法
```shell
pip install bubble_env
```
...
```yml
chain:
  bubble: env-files/bin/bubble  # bubble二进制
  ...
  init:
    ...
    ssh_port: 22
    p2p_port: 16789
    options: '--graphql --http --http.addr 0.0.0.0 --http.port 6789 --http.api web3,bub,txpool,net,admin,personal,debug --http.ethcompatible --ws --ws.addr 0.0.0.0 --ws.port 6790 --wsapi web3,bub,txpool,net,admin,personal,debug --ws.origins "*" --allow-insecure-unlock --txpool.nolocals --nodiscover --db.nogc --debug --verbosity 5'
    nodes:
  ...
  normal:
    ...
    ssh_port: 22
    p2p_port: 17789
    options: '--graphql --http --http.addr 0.0.0.0 --http.port 7789 --http.api web3,bub,txpool,net,admin,personal,debug --http.ethcompatible --ws --ws.addr 0.0.0.0 --ws.port 7790 --wsapi web3,bub,txpool,net,admin,personal,debug --ws.origins "*" --allow-insecure-unlock --txpool.nolocals --nodiscover --db.nogc --debug --verbosity 5'
    nodes:
      - host: 192.168.120.121
