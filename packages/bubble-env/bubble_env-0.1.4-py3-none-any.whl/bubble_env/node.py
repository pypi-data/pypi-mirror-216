...
class Node(Process):
    ...
    def start(self, options: str = ''):
        """ 使用supervisor启动节点
        """
        options = options or self.options  # 在添加supervisor配置的时候用到
        self.host.supervisor.add(self.name, self.supervisor_config(options))
        self.host.supervisor.start(self.name)
        time.sleep(3)  # 等待节点真正启动完成
    ...
    def supervisor_config(self, options=''):
