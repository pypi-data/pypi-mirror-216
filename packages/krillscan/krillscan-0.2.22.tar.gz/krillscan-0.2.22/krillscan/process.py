...
class krillscan_class():
    ...
    def read_raw(self,rawfile):       
        ...
        # # fix single ping data storage issues
        # for f in raw_freq:
        ...
        #         # cal_obj = raw_data[k] .get_calibration()

        #         # sv_obj = raw_data[k] .get_sv()    
        #         # sv_obj = raw_data.get_sv()   

                
        #         # plt.figure(2)
        #         # plt.clf()
        #         # plt.plot(                  raw_data.power)
        #         sv_mat = np.empty([ raw_data[0].n_samples , raw_obj.n_pings ])
                
        #         for k in range(len(raw_data)):
        #             sv_mat[:,k]    =          raw_data[k].power
